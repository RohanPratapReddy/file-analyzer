# `src/shell` — shell / command-language / automation-script analyzers folded into `code`

**Package:** `src/shell` · **Import:** `from src import ShellScriptAnalyzer` (and the individual analyzers, e.g. `from src import PosixShellAnalyzer`) · **Component:** `code` / `polyglot` (shell files are analyzed as part of the code plane) · `<ClassName>` for a single dialect

## What it does

`src/shell` provides real, syntax-aware analyzers for the shell / command-language
...
The package mirrors `src/prog_lang` exactly:
...
`SHELL_EXT_MAP` (in `src/shell/__init__.py`) is `ext → analyzer class`. It is not
...
`from src import SHELL_EXT_MAP; sorted(SHELL_EXT_MAP)`.
...
```bash
python -m src.main . --component PosixShellAnalyzer --emit posix.json
python -m src.main --list-components         # PosixShellAnalyzer, TclAnalyzer, ...
```
...
```python
from src import ShellScriptAnalyzer     # standalone dispatcher over SHELL_EXT_MAP
...
from src import PosixShellAnalyzer
...
`src/prog_lang/polyglot.py`, at module bottom, does
...
- [../script/README.md](../script/README.md) — the sibling `src/script` fold-in.
