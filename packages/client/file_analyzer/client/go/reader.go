// client-view-reader (Go): concurrent, strictly READ-ONLY viewer for a SQLite
// database a previous analysis run produced. This is the *client wheel's own*
// copy of the concurrent view reader, so file-analyzer-client is self-sufficient:
// it drives Go concurrency for bulk view reads without needing the engine wheel
// installed. It is byte-for-byte equivalent in behaviour to the engine's
// file_analyzer/views/go/main.go and is driven by file_analyzer.client.native_reader.
//
// This program is a WORKER over the analysis VIEWS installed into the database
// (the v_* objects). It embeds no query SQL of its own: it discovers the
// installed VIEW objects from the database catalog and reads them
// (SELECT * FROM "<view>"), so any view added on the write side is picked up here
...
func main() {
	...
	// Machine-readable mode: used by the Python launcher (file_analyzer/views/native_reader.py).
	...
	if len(viewNames) == 0 {
		...
		fmt.Println("install them first, e.g.:  python -m file_analyzer.views install", *source)
...
// database (built from file_analyzer/views/catalog.py). This replaces the previously
