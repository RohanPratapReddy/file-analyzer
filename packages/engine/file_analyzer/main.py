"""
tabgen ``file_analyzer`` package entry point.

By default this runs the full analysis pipeline (``file_analyzer.core.AnalysisEngine``)
...
Both the Go view-reader (``file_analyzer/views/go``) and the Postgres docker
...
Run it either way -- both resolve the ``file_analyzer`` package identically:
    ...
    python /app/readers/file_analyzer/main.py <source-dir> --out /artifacts
...
def _bootstrap_package_root() -> None:
    """Make ``import file_analyzer`` resolve no matter how this file was launched.

    ``python -m file_analyzer.main`` already has the readers/ dir on ``sys.path``; running
    the file directly (``python .../file_analyzer/main.py``) does not, so prepend the
...
from file_analyzer.core.analysis_engine import (  # noqa: E402  (after bootstrap)
    AnalysisEngine,
)
...
def _lang_analyzer_registry() -> dict:
    ...
    ``PythonCodeAnalyzer``.
    """
    from file_analyzer.prog_lang.polyglot import PolyglotCodeAnalyzer
...
def build_parser() -> argparse.ArgumentParser:
    ...
    conc.add_argument(
        ...
        type=int,
        default=None,
        help="workers for the Go analysis plane (per-shard fan-out); "
...
def run_component(args) -> int:
    ...
    if key in ("census", "repo", "repositoryanalyzer"):
        ...
        if args.mapping:
            from file_analyzer.router.routing import build_mapping, group_into_shards
    ...
    from file_analyzer.router.routing import reconstruct_paths, resolve_analyzer
    ...
    if plane is not None:
        ...
        import file_analyzer.engine as _src
...
def run(argv=None) -> int:
    ...
    require_virtualized(context="file_analyzer.main")
