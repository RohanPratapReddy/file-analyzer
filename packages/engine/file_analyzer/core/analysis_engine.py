...
``AnalysisEngine`` is the single entry point that runs the whole tabgen flow:
    ...
    RepositoryAnalyzer            file census + file->analyzer mapping (temp/)
        ...
        v
    router plane (Go)             parallel/concurrent per-shard analysis; each
...
The router package (``file_analyzer.router``) owns the routing and the language planes; this
class owns the orchestration and lives in ``file_analyzer/core`` and is re-exported at the
flat public API as ``from file_analyzer import AnalysisEngine``.
"""
...
class AnalysisEngine:
    ...
    def __init__(
        ...
        # stages -- the routing plane (Go per-shard fan-out) and the
        ...
        # The 'readers' directory is the parent of the 'file_analyzer' package, and is the
        # sys.path root that makes 'from file_analyzer import ...' resolve inside workers.
        # This module lives at file_analyzer/core/analysis_engine.py, so readers/ is 2 up.
    ...
    def run(self) -> Dict[str, Any]:
        ...
        # 2. Analysis plane (Go, concurrent) fans out per-shard workers.
        ...
        #     archives -- not driven by the Go shard workers.
        ...
        #     the Go reader can enumerate & read them with no embedded
