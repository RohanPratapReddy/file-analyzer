...
The FIFO diff database (:mod:`file_analyzer.monitor.diff_db`) only ever keeps the last
...
Because it is built on :class:`file_analyzer.monitor.store.SqlStore`, the change log can
...
statement are dialect-portable (see :mod:`file_analyzer.monitor.store`).
