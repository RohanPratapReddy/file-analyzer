...
class TradeStationTSLAnalyzer(RegexCodeAnalyzer):
    ...
    _SECTION = re.compile(
        r"(?mis)\b(inputs?|variables?|vars?|arrays?|array)\s*:" r"\s*(.*?);"
    )
    _METHOD = re.compile(r"(?mi)^[ \t]*Method\s+(?:" + _ID + r")\s+(" + _ID + r")\s*\(")
    ...
    def _extract_entities(self, file_id, text, path):
        ...
        for sec in self._SECTION.finditer(clean):
            ...
            scope = (
                ...
                else "input" if kind == "input" else "variable"
            )
        ...
        for m in self._METHOD.finditer(clean):
            ...
            self._add_function(file_id, name, [], [], description="EasyLanguage method")
