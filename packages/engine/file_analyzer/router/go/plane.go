...
//
// ROLE
//   This is the "plane" the router uses to run the repository's analyzers in
//   parallel (with a pure-Python fallback pool when Go is absent). The routing
//   decision (which file goes to
...
func main() {
	...
	readersRoot := flag.String("readers-root", "", "path to the 'readers' directory (so 'from file_analyzer import ...' resolves)")
	...
	workerScript := filepath.Join(*readersRoot, "file_analyzer", "router", "worker.py")
