...
def _toolchains() -> Dict[str, bool]:
    """Report which native toolchains are on PATH.

    The analysis planes (``file_analyzer.router``) and the view readers (``file_analyzer.views``) both
    speed up with Go present and transparently fall back to concurrent Python when
    absent. Surfacing this lets an agent know whether it actually got the native
    fast path.
    """
    import shutil
...
def server_info() -> Dict[str, Any]:
    ...
    the Go native fast path is available on this host (it falls back to concurrent
    Python when absent).
    """
    return {
        "name": "file-analyzer",
...
def analyze_repository(
    ...
    The analysis planes fan out across concurrent Go workers when the toolchain
    is on PATH (falling back to concurrent Python otherwise); ``plane_workers``
    and ``injection_workers`` tune that stage, and the returned ``toolchains``
    field reports whether the native fast path was actually available.
    ...
    Args:
        ...
        plane_workers: Concurrency for the Go analysis plane (default: CPU count).
...
def read_views(
    ...
    """Bulk-read the ``v_*`` analysis views in one call, using the concurrent Go reader.

    Reads many views at once instead of issuing a ``query`` per view. With
    ``engine="auto"`` (default) it hands the view set to the native Go reader,
    which fans out across a goroutine pool -- transparently falling back to a
    concurrent pure-Python read when the Go toolchain is not on PATH. Every read
    ...
    Args:
        ...
        engine: "auto" (native, else Python), "native" (Go, error if absent),
            or "python" (concurrent in-process, no toolchain needed).
        workers: Worker count (default: CPU count).

    Returns ``{"engine", "used", "view_count", "views": {name: {columns, rows} |
    {error}}}``. ``engine`` reports what actually ran ("go" or "python").
    """
    ...
    with _hushed():
        if eng in ("auto", "native"):
            ...
            if result is None and eng == "native":
                raise RuntimeError(
                    "native reader unavailable: the Go toolchain is not on PATH "
...
def warm_cache() -> None:
    ...
    try:
        with _hushed():
            ...
            import file_analyzer.engine  # noqa: F401  (loads the whole analyzer fleet)
...
def main() -> None:
    ...
    if "--precompile" in argv:
        warm_cache()
        return
    # Containment guard: serving the MCP tools runs the analysis engine, so it is
    # only permitted inside a container or a virtual machine -- never on bare-metal
    # host hardware (refuses with exit code 3 unless FILE_ANALYZER_ALLOW_BARE_METAL
    # is set). This mirrors the file_analyzer.main / `python -m file_analyzer` CLIs.
    # The SDK import path (`import file_analyzer`) stays unguarded on purpose;
    # `--version` and `--precompile` above also exit before this, so the handshake
    # probe and the build-time warm-up still work anywhere (e.g. `docker build`,
    # where /.dockerenv is not yet present).
    from file_analyzer.runtime_guard import require_virtualized
