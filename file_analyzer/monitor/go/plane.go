...
// (file_analyzer/monitor/worker.py, i.e. `python -m file_analyzer.monitor.worker`).
//
// ROLE
...
//
//       python -m file_analyzer.monitor.worker --readers-root <root> --out <out> --batch <f>
//
...
func main() {
	...
	readersRoot := flag.String("readers-root", "", "path to the 'readers' directory (so 'import file_analyzer' resolves)")
	...
	workerScript := filepath.Join(*readersRoot, "file_analyzer", "monitor", "worker.py")
...
// is the readers root so `python -m file_analyzer.monitor.worker` resolves the package.
func runWorker(python, readersRoot, script, out, id string) error {
	batchFile := filepath.Join(out, "batches", "batch_"+id+".json")
	cmd := exec.Command(python, "-m", "file_analyzer.monitor.worker",
