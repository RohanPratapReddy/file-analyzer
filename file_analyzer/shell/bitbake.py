...
class BitBakeAnalyzer(ShellScriptBase):
    ...
    def _extract_entities(self, file_id, text, path):
        ...
        for m in self._VARSET.finditer(clean):
            name = m.group(1)
            if name in seen_var or name in (
                "inherit",
                "require",
                "include",
                "addtask",
            ...
            seen_var.add(name)
            self._add_variable(
                file_id, name, (m.group(3) or "").strip()[:120] or None, scope="recipe"
            )
        ...
        for m in self._SHTASK.finditer(clean):
            ...
            if name and name not in seen_fn and name not in ("python", "def"):
                ...
                self._add_shell_function(
                    file_id, name, description="bitbake shell task"
                )
        for m in self._PYTASK.finditer(clean):
            ...
            if name not in seen_fn:
                seen_fn.add(name)
                self._add_shell_function(
                    file_id, name, description="bitbake python task"
                )
        for m in self._PYDEF.finditer(clean):
            ...
            if name not in seen_fn:
                ...
                self._add_shell_function(
                    file_id, name, params=params, description="bitbake python def"
                )
        ...
        self._record_module_meta(
            file_id,
            recipe_kind=path.suffix.lower().lstrip("."),
            added_tasks=tasks or None,
            exported_functions=self._uniq(exported) or None,
