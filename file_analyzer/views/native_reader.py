...
and Java *analysis* planes). The Go program (``file_analyzer/views/go/main.go``) and the
Java program (``file_analyzer/views/java/RepositoryReader.java``) can each read every
...
_JAVA_DIR = _READERS_ROOT / "file_analyzer" / "views" / "java"
...
def _java_classpath(log: List[str]) -> Optional[str]:
    ...
    if not jars:
        log.append(
            "[native-read] java: no JDBC/SLF4J jars found in file_analyzer/views/java"
        )
        return None
