...
_ATTRS = r"(?:\[[^\]]*\]\s*)*"
_MODS = (
    r"(?:public\s+|private\s+|protected\s+|internal\s+|static\s+|abstract\s+|"
...
class NemerleAnalyzer(RegexCodeAnalyzer):
    ...
    _METHOD = re.compile(
        r"(?m)^\s*" + _ATTRS + _MODS + r"(" + _ID + r")\s*(?:\[[^\]]*\])?\s*\("
    )
    ...
    def _extract_entities(self, file_id, text, path):
        ...
        for m in self._NAMESPACE.finditer(clean):
            self._add_class(
                file_id, m.group(1).split(".")[-1], description="nemerle namespace"
            )
