...
class RegoAnalyzer(RegexCodeAnalyzer):
    ...
    _IMPORT = re.compile(
        r"^\s*import\s+([\w.]+)(?:\s+as\s+(" + _NAME + r"))?", re.MULTILINE
    )
    ...
    def _extract_entities(self, file_id, text, path):
        ...
        for m in self._RULE.finditer(text):
            ...
            if params is not None:
                ...
                arity_key = (name, len([p for p in self._split_top_level(params) if p]))
                if arity_key in seen:
                    continue
                seen.add(arity_key)
                rule_names.add(name)
                arg_ids = [
                    self._add_arg(p.strip())
                    for p in self._split_top_level(params)
                    if p.strip()
                ]
                self._add_function(
                    file_id, name, arg_ids, [], description="rego function"
                )
            else:
                if name in seen:
                    continue
