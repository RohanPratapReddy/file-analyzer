...
class JavaAnalyzer(BaseTreeSitterAnalyzer):
    ...
    def _register_types(self, root_node, source: bytes):
        for child in root_node.children:
            if child.type in [
                ...
                if name_node:
                    self._class_registry[self._node_text(name_node, source)] = (
                        self._class_counter
                    )
    ...
    def _parse_class(self, file_id: int, node, source: bytes):
        ...
        if superclass_node:
            pname = (
                self._node_text(superclass_node, source).replace("extends ", "").strip()
            )
        ...
        if body_node:
            for member in body_node.children:
                if member.type == "field_declaration":
                    ...
                    decls = [
                        c for c in member.children if c.type == "variable_declarator"
                    ]
                    for decl in decls:
                        ...
                        self.args_table.append(
                            {
                                "args_id": aid,
                                "args_name": (
                                    self._node_text(dname, source)
                                    if dname
                                    else "anon_field"
                                ),
                                "args_type": (
                                    self._node_text(ftype, source)
                                    if ftype
                                    else "Object"
                                ),
                                "default_value": None,
        ...
        self.record_introspection_metadata(
            ...
            structural_properties={
                "declared_fields": len(attr_ids),
                "declared_methods": len(method_ids),
            },
        )
                    ...
                    self.args_table.append(
                        {
                            ...
                            "args_type": (
                                self._node_text(ptype, source) if ptype else "Object"
                            ),
                            "default_value": None,
                            "permitted_values": None,
                        }
