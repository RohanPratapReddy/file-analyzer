...
class LuaAnalyzer(RegexCodeAnalyzer):
    ...
    def _extract_entities(self, file_id, text, path):
        ...
        for m in self._FUNC.finditer(t):
            ...
            self._add_function(file_id, fname, arg_ids, class_id=cls_id,
                               description="lua function" if not local
                               else "lua local function")
            emitted_fn.add(name)
        for m in self._ASSIGN_FUNC.finditer(t):
            name, params = m.group(2), m.group(3)
            if name in emitted_fn:
                continue
            self._add_function(file_id, name.split(".")[-1], self._args(params),
