...
class DartAnalyzer(RegexCodeAnalyzer):
    ...
    _IMPORT = re.compile(
        r"""^\s*(?:import|export|part)\s+['"]([^'"]+)['"]""" r"""(?:\s+as\s+(\w+))?""",
        re.MULTILINE,
    )
    _CLASS = re.compile(
    ...
    _MIXIN = re.compile(r"^\s*mixin\s+(\w+)(?:\s+on\s+([\w.,\s]+))?", re.MULTILINE)
    ...
    def _extract_entities(self, file_id, text, path):
                ...
                if grp:
                    for p in grp.split(","):
                        ...
                        if pid is not None:
                            parents.append(pid)
            self._add_class(
                file_id, m.group(1), description="dart class", parent_ids=parents
            )
        ...
        for m in self._ENUM.finditer(t):
            attr_ids = [
                self._add_arg(v.strip().split("(")[0])
                for v in m.group(2).split(",")
                if v.strip()
            ]
            self._add_class(
                file_id, m.group(1), description="dart enum", attr_ids=attr_ids
            )
        ...
        for m in self._FUNC.finditer(t):
            name = m.group(1)
            if (
                name in self._KEYWORDS
                or name[0].isupper()
                and name in self._class_registry
            ):
                continue
