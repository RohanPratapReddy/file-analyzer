...
class MoonScriptAnalyzer(RegexCodeAnalyzer):
    ...
    _IMPORT = re.compile(
        ...
        re.MULTILINE)
    _CLASS = re.compile(r"^([ \t]*)(?:export\s+)?class\s+([A-Za-z_]\w*)"
    ...
    _EXPORT = re.compile(r"^[ \t]*export\s+(?!class\b|\*|\^|default\b)"
    ...
    def _extract_entities(self, file_id, text, path):
        ...
        for m in self._CLASS.finditer(text):
            ...
            for mm in self._METHOD.finditer(body):
                ...
                arg_ids = [self._add_arg(p.strip().lstrip("@"))
                           for p in params.split(",") if p.strip()]
                methods.append(self._add_function(
                    file_id, mm.group(2), arg_ids, [],
                    class_id=self._class_registry.get(name),
                    description="moonscript method"))
            self._add_class(file_id, name, description="moonscript class",
        ...
        for m in self._EXPORT.finditer(text):
            ...
            for nm in names:
                if single_fn:
                    ...
                    arg_ids = [self._add_arg(p.strip().lstrip("@"))
                               for p in params.split(",") if p.strip()]
                    self._add_function(file_id, nm, arg_ids, [],
                                       description="moonscript function")
                else:
