...
class CythonAnalyzer(RegexCodeAnalyzer):
    ...
    _CIMPORT = re.compile(r"^[ \t]*cimport\s+([\w.]+)(?:\s+as\s+(" + _ID + r"))?",
                          re.MULTILINE)
    _FROM_CIMPORT = re.compile(r"^[ \t]*from\s+([\w.]+)\s+cimport\s+(.+)$",
                               re.MULTILINE)
    _IMPORT = re.compile(r"^[ \t]*import\s+([\w.]+)(?:\s+as\s+(" + _ID + r"))?",
                         re.MULTILINE)
    _FROM_IMPORT = re.compile(r"^[ \t]*from\s+([\w.]+)\s+import\s+(.+)$",
                              re.MULTILINE)
    _EXTERN = re.compile(r'^[ \t]*cdef\s+extern\s+from\s+["<]([^">]+)[">]',
                         re.MULTILINE)

    _CLASS = re.compile(r"^([ \t]*)(?:cdef\s+|cpdef\s+)?class\s+(" + _ID +
                        r")\s*(?:\(([^)]*)\))?\s*:", re.MULTILINE)
    # C++ interop:  [cdef [extern] [api]] cppclass Name[T](Base):
    _CPPCLASS = re.compile(r"^([ \t]*)(?:cdef\s+)?(?:extern\s+)?(?:api\s+)?"
                           r"cppclass\s+(" + _ID + r")\s*(?:\[[^\]]*\])?"
                           r"\s*(?:\(([^)]*)\))?\s*:", re.MULTILINE)
    _CTYPEDEF = re.compile(r"^[ \t]*ctypedef\b(?!\s+(?:struct|union|enum|"
                           r"cppclass|class)\b)[^\n]*?\b(" + _ID + r")\s*$",
                           re.MULTILINE)
    _CTYPEDEF_REC = re.compile(r"^[ \t]*ctypedef\s+(?:struct|union|enum|cppclass)"
                               r"\s+(" + _ID + r")", re.MULTILINE)
    ...
    _FUNC = re.compile(r"^([ \t]*)(?:(cdef|cpdef)\s+|def\s+)"
                       r"(?:[\w\*\[\]\., ]+?\s+)??"      # optional return type
                       r"(" + _ID + r")\s*\(", re.MULTILINE)

    # cdef data declaration:  cdef <type> name [, name...]  (no '(' before EOL)
    _CVAR = re.compile(r"^([ \t]*)cdef\s+(?!class\b|extern\b|inline\b|packed\b)"
                       r"([\w\*\[\]\.\": <>]+?\s+)(" + _ID +
                       r"(?:\s*,\s*" + _ID + r")*)\s*(?:=[^\n]*)?$", re.MULTILINE)
    ...
    def _extract_entities(self, file_id, text, path):
        for m in self._CIMPORT.finditer(text):
            self._add_import(file_id, m.group(2) or m.group(1).split(".")[-1],
                             m.group(1), m.group(2))
        ...
        for indent, name, bases, cid, a, b in class_spans:
            ...
            for p in self._split_top_level(bases or ""):
                ...
                if p:
                    ...
                    if pid is not None:
                        parents.append(pid)
            self._add_class(file_id, name, description="cython class",
            ...
            for raw in m.group(3).split(","):
                ...
                if nm:
                    self._add_variable(file_id, nm,
                                       scope="attr" if oid is not None else "module")
    ...
    _CPP_QUALS = re.compile(r"\b(?:nogil|const)\b|except\s*\+?[^\n]*|"
    ...
    def _scan_cppclass_members(self, file_id, text):
        ...
        for m in self._CPPCLASS.finditer(text):
            ...
            for k in range(li + 1, len(lines)):
                ...
                body = raw.strip()
                if body.startswith(("#", "cppclass", "cdef", "ctypedef",
                                    "pass", "ns_", "@")):
                    continue
                ...
                if "(" in body:                 # a method / ctor signature
                    ...
                    arg_ids = self._parse_params(raw[lp + 1:ap - 1]) if ap > lp else []
    ...
    def _class_spans(self, text):
        ...
        for ln in lines:
            offsets.append(pos)
            pos += len(ln)
        matches = list(self._CLASS.finditer(text)) + \
            list(self._CPPCLASS.finditer(text))
