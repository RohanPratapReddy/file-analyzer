...
class BaseTreeSitterAnalyzer(BaseCodeAnalyzer):
    ...
    def __init__(self, lang_key: str, extensions: List[str],
                 introspection_source: Optional[str] = None, **kwargs):
        super().__init__(language_name=lang_key, **kwargs)
        self.lang_key = lang_key
        self.extensions = extensions
        self.introspection_source = introspection_source
        if get_parser is None:
            raise ImportError("tree-sitter-languages is required. Run: pip install tree-sitter-languages")
    ...
    def _ts_add_variable(self, file_id, name, value=None, scope="module",
    ...
    def _ts_add_function(self, file_id, name, arg_ids=None, output_ids=None,
    ...
    def _ts_add_class(self, file_id, name, description=None, parent_ids=None,
                      method_ids=None, attr_ids=None, is_imported=False,
                      source_import_id=None, introspect=True):
        ...
        row = {
            ...
            "args_ids": [], "attr_ids": attr_ids or [], "tensor_member_ids": [],
            "is_imported": is_imported, "source_import_id": source_import_id,
        }
        ...
        if introspect:
            self.record_introspection_metadata(
                ...
                inspection_source=self.introspection_source or getattr(self, "lang_key", "generic"),
    ...
    def analyze(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        valid_files = [p for p in self.file_paths if p.suffix.lower() in exts and p.exists()]
