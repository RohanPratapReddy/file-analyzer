        ...
        for m in self._TYPE.finditer(text):
            ...
            if ctor:
                for part in self._split_top_level(ctor):
                    pm = re.match(r"(?:mutable\s+)?([A-Za-z_]\w*)\s*(?::\s*(.+))?",
                                  part.strip())
                    if pm:
                        attrs.append(self._add_arg(
                            pm.group(1),
            ...
            if "{" in head:
                ...
                for fld in re.split(r"[;\n]", text[lb + 1:rb - 1]):
                    fm = re.match(r"\s*(?:mutable\s+)?([A-Za-z_]\w*)\s*:\s*(.+)",
                                  fld)
                    if fm:
            ...
            self._add_class(file_id, name, description="fsharp type",
                            attr_ids=attrs, method_ids=methods)

        # collect member spans so top-level `let` scan skips members (they are
        # matched as class methods above)
        member_lines = {text.count("\n", 0, mm.start())
                        for mm in self._MEMBER.finditer(text)}
