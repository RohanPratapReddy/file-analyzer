...
class MoveAnalyzer(RegexCodeAnalyzer):
    ...
    _USE = re.compile(r"\buse\s+([\w:]+)(?:::\{([^}]*)\})?(?:\s+as\s+(\w+))?\s*;",
                      re.MULTILINE)
    ...
    def _extract_entities(self, file_id, text, path):
        ...
        for m in self._USE.finditer(text):
            ...
            if group:
                for item in self._split_top_level(group):
                    ...
                    if am:
                        self._add_import(file_id, am.group(2),
                                         f"{base}::{am.group(1)}", am.group(2))
                    elif it == "Self":
                ...
                for fld in self._split_top_level(body):
                    ...
                    if fm:
                        attrs.append(self._add_arg(fm.group(1),
                                                   fm.group(2).strip()[:80]))
            self._add_class(file_id, name, description="move struct",
                            attr_ids=attrs)
        ...
        for m in self._FUN.finditer(text):
            ...
            for part in self._split_top_level(params):
                pm = re.match(r"(?:mut\s+)?([A-Za-z_]\w*)\s*:\s*(.+)", part.strip(),
                              re.DOTALL)
                if pm:
                    arg_ids.append(self._add_arg(pm.group(1),
                                                 pm.group(2).strip()[:80]))
            # return type after ')' up to '{' or 'acquires' or ';'
            tail = text[rp:text.find("{", rp) if text.find("{", rp) != -1
                        else rp + 200]
            out_ids = []
