...
class AgdaAnalyzer(RegexCodeAnalyzer):
    ...
    _IMPORT = re.compile(
        ...
        r"(?:\s+as\s+([\w.]+))?(?:.*?\busing\s*\(([^)]*)\))?", re.MULTILINE)
    ...
    def _extract_entities(self, file_id, text, path):
        ...
        for m in self._DATA.finditer(text):
            ...
            cons = []
            for cm in re.finditer(r"^\s+(" + _NAME + r")\s*:(?!:)", body,
                                  re.MULTILINE):
        ...
        for m in self._RECORD.finditer(text):
            ...
            if fm:
                ...
                for f in re.finditer(r"^\s*(" + _NAME + r")\s*:(?!:)", fblock,
                                     re.MULTILINE):
                    attrs.append(self._add_arg(f.group(1), "field"))
            self._add_class(file_id, name, description="agda record",
                            attr_ids=attrs)

        # postulate blocks -> declared names become functions
        for m in self._POSTULATE.finditer(text):
            body = self._indent_body(text, m.start())
            for pm in re.finditer(r"^\s*(" + _NAME + r")\s*:(?!:)", body,
                                  re.MULTILINE):
                self._add_function(file_id, pm.group(1), [], [],
                                   description="agda postulate")

        # top-level signatures (indent 0) + definitions
        reserved = {"module", "where", "data", "record", "field", "postulate",
                    "open", "import", "using", "hiding", "renaming", "with",
                    "mutual", "abstract", "private", "instance", "primitive",
                    "infixl", "infixr", "infix", "syntax", "let", "in"}
        sigs = {}
        ...
        for name, sig in sigs.items():
            out_ids = ([self._add_output(sig.split("->")[-1].strip()[:80])]
                       if "->" in sig or "→" in sig else [])
            if "->" in sig or "→" in sig:
                self._add_function(file_id, name, [], out_ids,
                                   description="agda function")
            else:
