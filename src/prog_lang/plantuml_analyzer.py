...
class PlantUMLAnalyzer(RegexCodeAnalyzer):
    ...
    _PACKAGE = re.compile(r"(?im)^\s*(?:package|namespace|frame|together)\s+(" + _QID + r")")
    _INCLUDE = re.compile(r"(?im)^\s*!include(?:url|sub)?\s+(.+)$")
    _FUNC = re.compile(r"(?im)^\s*!(?:unquoted\s+)?(?:function|procedure)\s+\$?(" + _ID + r")\s*\(")
    _DEFINE = re.compile(r"(?im)^\s*!define\s+(" + _ID + r")\b")
    _VAR = re.compile(r"(?im)^\s*!\$(" + _ID + r")\s*=")
    _EXTENDS = re.compile(r"(?im)^\s*(" + _QID + r")\s+(?:extends|--\|>|<\|--)\s+(" + _QID + r")")
    ...
    def _extract_entities(self, file_id, text, path):
        ...
        for m in self._PACKAGE.finditer(clean):
            self._add_class(file_id, self._unq(m.group(1)),
                            description="plantuml package")
        for m in self._ACTOR.finditer(clean):
            self._add_class(file_id, self._actor_name(m.group(2), m.group(3)),
        ...
        for m in self._TYPE.finditer(clean):
            ...
            if m.group(4):  # brace body present
                ...
                for line in body.split("\n"):
                    if not line.strip() or line.strip().startswith(("..", "--", "==", "__")):
                        continue
                    ...
                    if mm.group(2):
                        methods.append(self._add_function(
                            file_id, mm.group(1), [], [], description="plantuml method"))
                    else:
                        attrs.append(self._add_arg(mm.group(1)))
            self._add_class(file_id, name, description="plantuml class",
