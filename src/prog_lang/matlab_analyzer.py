...
class MatlabAnalyzer(RegexCodeAnalyzer):
    ...
    _CLASSDEF = re.compile(
        r"^\s*classdef\s+(?:\([^)]*\)\s*)?(\w+)(?:\s*<\s*([\w.&\s]+))?", re.MULTILINE
    )
    ...
    _FUNC_NORET = re.compile(r"^\s*function\s+(\w+)\s*\(([^)]*)\)\s*$", re.MULTILINE)
    ...
    def _extract_entities(self, file_id, text, path):
        ...
        for m in self._IMPORT.finditer(t):
            self._add_import(
                file_id, m.group(1).split(".")[-1].rstrip(".*"), m.group(1)
            )
        ...
        for m in self._FUNC.finditer(t):
            ...
            arg_ids = [self._add_arg(a.strip()) for a in params.split(",") if a.strip()]
            self._add_function(
                file_id,
                name,
                arg_ids,
