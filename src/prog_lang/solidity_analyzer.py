...
class SolidityAnalyzer(RegexCodeAnalyzer):
    ...
    def _extract_entities(self, file_id, text, path):
        ...
        for m in self._PRAGMA.finditer(t):
            self._add_variable(file_id, "pragma", m.group(1).strip(), scope="directive")
                ...
                for b in bases.split(","):
                    ...
                    if b:
                        ...
                        if pid is not None:
                            parents.append(pid)
            self._add_class(
                file_id, name, description=f"solidity {kind}", parent_ids=parents
            )
        ...
        for m in self._STRUCT.finditer(t):
            ...
            for fld in m.group(2).split(";"):
                ...
                if fm:
                    attr_ids.append(self._add_arg(fm.group(2), fm.group(1).strip()))
            self._add_class(
                file_id, m.group(1), description="solidity struct", attr_ids=attr_ids
            )
        for m in self._ENUM.finditer(t):
            attr_ids = [
            ...
            self._add_class(
                file_id, m.group(1), description="solidity enum", attr_ids=attr_ids
            )
