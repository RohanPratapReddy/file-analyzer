...
class DataAnalyzer:
    ...
    MAX_STRUCT_TEXT_BYTES = 128 * 1024 * 1024  # full-text load ceiling (XML/JSON structs)
    MAX_GEOM_VERTS = 5_000_000  # vertices sampled for a bounding box (count stays exact)

    # ---- curated extension routing (checked before catalog fallback) --
    _EXT_DELIM = {
        ".csv": ",", ".tsv": "\t", ".tab": "\t", ".psv": "|", ".ssv": " ",
        ".scsv": ";", ".dsv": None, ".dat": None, ".prn": None, ".rec": None,
    }
    ...
    _EXT_IMAGE = {
        ...
        ".tif", ".tiff", ".webp", ".ppm", ".pgm", ".pbm", ".pnm", ".tga",
        ".ico", ".icns", ".jp2", ".j2k", ".jpf", ".jpx", ".heic", ".heif",
        ".avif", ".dds", ".im", ".pcx", ".sgi", ".xbm", ".xpm", ".msp",
    }
    ...
    _EXT_GEORASTER = {
        ".vrt", ".asc", ".grd", ".dem", ".ddf", ".bil", ".bip", ".bsq",
        ".flt", ".dt0", ".dt1", ".dt2", ".nc", ".cdf", ".nc4",
    }
    ...
    _EXT_RAW = {
        ...
        ".dng", ".raf", ".rw2", ".orf", ".pef", ".ptx", ".srw", ".3fr",
        ".dcr", ".kdc", ".mrw", ".x3f", ".iiq", ".mef", ".rwl", ".erf",
        ".mos", ".fff", ".gpr", ".raw",
    }
    ...
    _TYPE_TO_CATEGORY = {
        "image_raster": "image", "image_raw": "image", "image_vector": "image",
    ...
    def _known_exts(self) -> set:
        if not hasattr(self, "_known_ext_cache"):
            known = set()
            for grp in (
                self._EXT_DELIM.keys(), self._EXT_PARQUET, self._EXT_ORC,
                self._EXT_ARROW, self._EXT_EXCEL, self._EXT_JSON_REC,
                self._EXT_SQLITE, self._EXT_STAT, self._EXT_SAFETENSORS,
                self._EXT_NPY, self._EXT_NPZ, self._EXT_TORCH, self._EXT_GGUF,
                self._EXT_ONNX, self._EXT_ORT, self._EXT_TFLITE, self._EXT_KERAS,
                self._EXT_FLAX, self._EXT_TFRECORD, self._EXT_PB,
                self._EXT_WAV, self._EXT_IMAGE, self._EXT_AUDIO,
                self._EXT_PDF, self._EXT_DOCX,
                self._EXT_VIDEO, self._EXT_TEXT, self._EXT_PICKLE,
                self._EXT_MESH, self._EXT_GEO, self._EXT_CONFIG,
                self._EXT_GRAPH, self._EXT_BIO, self._EXT_GML,
                self._EXT_GEORASTER, self._EXT_MEDICAL, self._EXT_RAW,
                self._EXT_POINTCLOUD,
                self._EXT_CATALOG_TEXT, self._EXT_CATALOG_BINARY,
    ...
    # Row builders
    # ==================================================================
    def _add_dataset(self, path: Path, ext: str, category: str, subcategory: str,
                     modality: str, local_fid: int, status: str = "ok",
    ...
    def _add_column(self, dataset_id: int, name: str, ordinal: int, data_type: str,
                    inferred_type: str, null_count: Optional[int],
                    non_null_count: Optional[int], unique_count: Optional[int],
                    min_value: Any, max_value: Any, mean_value: Optional[float],
                    std_value: Optional[float], sample_values: List[Any],
                    extra: Dict[str, Any], local_fid: int) -> int:
    ...
    def _add_tensor(self, dataset_id: int, name: str, dtype: Optional[str],
                    shape: List[int], num_bytes: Optional[int],
                    extra: Dict[str, Any], local_fid: int,
                    role: Optional[str] = None,
                    layer_name: Optional[str] = None) -> int:
    ...
    def _add_model_layer(self, dataset_id: int, layer_name: str, layer_type: str,
                         ordinal: int, depth: int, tensor_count: int,
                         param_tensor_count: int, buffer_tensor_count: int,
                         total_parameters: Optional[int],
                         total_buffer_elements: Optional[int],
                         total_bytes: Optional[int], dtypes: List[str],
                         param_shapes: Dict[str, Any], roles: Dict[str, str],
                         local_fid: int) -> int:
    ...
    def _add_relation(self, dataset_id: int, relation_type: str,
                      left_column: Optional[str], right_column: Optional[str],
                      value: Optional[float], method: Optional[str],
                      local_fid: int, extra: Optional[Dict[str, Any]] = None) -> int:
    ...
    def _add_property(self, dataset_id: int, name: str, value: Any,
                      group_name: str, local_fid: int) -> int:
    ...
    def _add_properties(self, dataset_id: int, props: Dict[str, Any],
    ...
    _PARAM_LEAF_ROLES = {
        ...
        "weight_g": "weight", "weight_v": "weight",
        "gamma": "weight", "beta": "bias",
        "in_proj_weight": "weight", "in_proj_bias": "bias",
        "q_proj_weight": "weight", "k_proj_weight": "weight",
        "v_proj_weight": "weight",
        "lora_a": "weight", "lora_b": "weight",
        "running_mean": "buffer", "running_var": "buffer",
        "num_batches_tracked": "buffer", "inv_freq": "buffer",
        "position_ids": "buffer", "token_type_ids": "buffer",
        "causal_mask": "buffer", "masked_bias": "buffer",
        "rotary_emb.inv_freq": "buffer",
        "scale": "quant_param", "zero_point": "quant_param",
        "weight_scale": "quant_param", "weight_zero_point": "quant_param",
    ...
    def _classify_param(self, tensor_name: str) -> Tuple[str, str]:
        ...
        if role is None and name.rsplit(".", 1)[-1].lower().endswith(("_scale", "_zero_point")):
            role = "quant_param"
    ...
    _LTYPE_TOKENS = (
        ("Embedding", ("embed", "wte", "wpe", "tok_emb", "word_embeddings",
                       "position_embeddings", "embeddings")),
        ("Convolution", ("conv", "patch_embed.proj", "downsample", "upsample")),
        ("Normalization", ("layernorm", "layer_norm", "rmsnorm", "rms_norm",
                            "batchnorm", "batch_norm", "groupnorm", "group_norm",
                            "ln_f", "ln1", "ln2", "ln_1", "ln_2", ".norm", "_norm",
                            "norm.", "input_layernorm", "post_attention_layernorm")),
        ("Attention", ("attn", "attention", "q_proj", "k_proj", "v_proj",
                       "o_proj", "out_proj", "qkv", "query", "key", "value",
                       "in_proj")),
        ("Linear/MLP", ("mlp", "ffn", "feed_forward", "gate_proj", "up_proj",
                        "down_proj", "fc1", "fc2", "fc_", "dense", "linear",
                        "proj", "lm_head", "classifier", "head")),
    )
    ...
    def _emit_model_layers(self, ds: Dict[str, Any], local_fid: int,
                           framework: str, tensor_start: int) -> None:
        ...
        for ordinal, layer in enumerate(order):
            ...
            self._add_model_layer(
                ds["dataset_id"], layer, ltype, ordinal, layer.count(".") + 1,
                len(trs), p_tensors, b_tensors,
                l_params or None, l_buffers or None,
                l_bytes if has_bytes else None, dtypes, pshapes, roles, local_fid)
        ...
        summary = {
            ...
            "parameters_by_layer_type": {k: v for k, v in ltype_params.items() if v} or None,
    ...
    def _emit_media_metrics(self, ds: Dict[str, Any], path: Path,
                            kind: str, local_fid: int) -> None:
    ...
    # ==================================================================
    def _profile_dataframe(self, df, ds_row: Dict[str, Any], local_fid: int,
                           exact_rows: Optional[int] = None) -> None:
        ...
        for ordinal, name in enumerate(cols, start=1):
            ...
            extra = {"stats_from_sample": exact_rows is None or n < (exact_rows or 0),
                     "sample_n": n}
            self._add_column(dataset_id, str(name), ordinal, dtype, inferred,
                             null_count, non_null, uniq, minv, maxv, meanv, stdv,
                             samples, extra, local_fid)
    ...
    def _emit_correlations(self, df, dataset_id: int, local_fid: int) -> None:
        ...
        for _, a, b, r in pairs[: self.CORR_MAX_RELATIONS]:
            self._add_relation(dataset_id, "pearson_correlation", a, b,
                               round(r, 6), "pearson", local_fid)
    ...
    def _compression_of(self, ext: str) -> Optional[str]:
        for suffix, comp in ((".gz", "gzip"), (".bz2", "bz2"),
                             (".zst", "zstd"), (".zstd", "zstd"),
                             (".xz", "xz")):
    ...
    def _handle_tabular_delimited(self, path, ext, category, subcategory, local_fid):
        ...
        ds = self._add_dataset(path, ext, category or "data",
        ...
        try:
            import pandas as pd
            df = pd.read_csv(path, sep=delim, nrows=self.SAMPLE_ROWS,
    ...
    def _handle_parquet(self, path, ext, category, subcategory, local_fid):
        ...
        self._add_properties(ds["dataset_id"], {
    ...
    def _schema_only_columns(self, arrow_schema, ds_row, local_fid):
        ...
        for ordinal, (name, typ) in enumerate(zip(names, types), start=1):
            self._add_column(ds_row["dataset_id"], str(name), ordinal, typ,
                             "unknown", None, None, None, None, None, None, None,
                             [], {"schema_only": True}, local_fid)
    ...
    def _handle_excel(self, path, ext, category, subcategory, local_fid):
        ...
        self._add_property(ds["dataset_id"], "sheet_names", wb.sheetnames,
                           "workbook", local_fid)
        self._add_property(ds["dataset_id"], "sheet_count", len(wb.sheetnames),
                           "workbook", local_fid)
        ws = wb.active
    ...
    def _handle_json(self, path, ext, category, subcategory, local_fid):
        ...
        if isinstance(obj, list):
            ...
            if records:
                ...
                ds["modality"] = "structured"
                self._add_property(ds["dataset_id"], "element_type",
            ...
            if (len(list_vals) >= 2 and len(list_vals) == len(obj)
            ...
            ds["modality"] = "scalar"
            self._add_property(ds["dataset_id"], "value_type",
                               type(obj).__name__, "json", local_fid)
    ...
    def _handle_sqlite(self, path, ext, category, subcategory, local_fid):
        ...
        try:
            ...
            for tname in tables:
                ds = self._add_dataset(path, ext, category or "data",
                                       "databases_and_dumps", "database_table",
                                       local_fid)
                ds["dataset_name"] = f"{path.name}::{tname}"
                self._add_property(ds["dataset_id"], "table_name", tname,
                                   "sqlite", local_fid)
                try:
                    ...
                    for ordinal, c in enumerate(cols, start=1):
                        ...
                        extra = {"not_null": bool(notnull),
                                 "primary_key": bool(pk),
                                 "default": dflt}
                        self._add_column(ds["dataset_id"], str(cname), ordinal,
                                         str(ctype), "unknown", None, None, None,
                                         None, None, None, None, [], extra, local_fid)
    ...
    def _handle_safetensors(self, path, ext, category, subcategory, local_fid):
        import struct
        ds = self._add_dataset(path, ext, category or "tensor_and_model",
        ...
        for name, info in header.items():
            ...
            try:
                ...
                pass
            self._add_tensor(ds["dataset_id"], name, dtype, shape, nbytes, {}, local_fid)
            count += 1
    ...
    def _handle_npy(self, path, ext, category, subcategory, local_fid):
        ...
        self._add_tensor(ds["dataset_id"], path.stem, dtype, shape, None,
                         {"fortran_order": fortran}, local_fid)
        ds["tensor_count"] = 1
        ...
        try:
            for member in zf.namelist():
                ...
                if info is None:
                    continue
                dtype, shape, fortran = info
                self._add_tensor(ds["dataset_id"], aname, dtype, shape, None,
                                 {"fortran_order": fortran}, local_fid)
                count += 1
        finally:
            zf.close()
    ...
    def _handle_torch(self, path, ext, category, subcategory, local_fid):
        import zipfile
        ds = self._add_dataset(path, ext, category or "tensor_and_model",
                               subcategory or "pytorch", "tensor", local_fid)
        t0 = len(self.data_tensors_table)
        ...
        if zipfile.is_zipfile(path):
            try:
                zf = zipfile.ZipFile(path)
                members = zf.namelist()
                data_members = [m for m in members if "/data/" in m or m.endswith("data.pkl")]
                self._add_properties(ds["dataset_id"], {
                    "container": "zip",
    ...
    def _walk_torch_obj(self, obj, ds, local_fid, prefix=""):
        ...
        def walk(o, pfx):
            ...
            if is_tensor(o):
                ...
                self._add_tensor(ds["dataset_id"], pfx or "tensor", dtype, shape,
                                 nbytes, {}, local_fid)
                count += 1
    ...
    def _handle_gguf(self, path, ext, category, subcategory, local_fid):
        ...
        if info.get("metadata"):
            self._add_properties(ds["dataset_id"], info["metadata"],
                                 "gguf_metadata", local_fid)
        count = 0
        for t in info.get("tensors", []):
            self._add_tensor(ds["dataset_id"], t["name"], t.get("dtype"),
                             t.get("shape", []), None,
                             {"offset": t.get("offset")}, local_fid)
            count += 1
    ...
    def _handle_onnx(self, path, ext, category, subcategory, local_fid):
        ...
        try:
            ...
            for init in g.initializer:
                ...
                self._add_tensor(ds["dataset_id"], init.name,
                                 dtname, shape, None, {}, local_fid)
                count += 1
            ds["tensor_count"] = count
    ...
    def _handle_tflite(self, path, ext, category, subcategory, local_fid):
        ...
        for i, sg in enumerate(info.get("subgraphs", [])):
            ...
            }, "tflite_subgraphs", local_fid)
        count = 0
        for t in info.get("tensors", []):
            self._add_tensor(ds["dataset_id"], t["name"], t.get("dtype"),
                             t.get("shape", []), None,
                             {"subgraph": t.get("subgraph")}, local_fid)
            count += 1
        ds["tensor_count"] = count
    ...
    def _handle_keras(self, path, ext, category, subcategory, local_fid):
        ...
        for ordinal, ly in enumerate(info.get("layers", [])):
            self._add_column(
                ds["dataset_id"], ly.get("name") or f"layer{ordinal}", ordinal,
                ly.get("class_name") or "Layer", "layer",
                None, None, None, None, None, None, None, [],
                {"class_name": ly.get("class_name")}, local_fid)
    ...
    def _handle_tfrecord(self, path, ext, category, subcategory, local_fid):
        ...
        for ordinal, feat in enumerate(info.get("features", [])):
            self._add_column(
                ds["dataset_id"], feat["name"], ordinal, feat.get("kind", "unknown"),
                "feature", None, None, None, None, None, None, None, [],
                {"value_count": feat.get("value_count")}, local_fid)
    ...
    def _handle_pb(self, path, ext, category, subcategory, local_fid):
        ...
        if graph is not None:
            self._add_properties(ds["dataset_id"], {
                "kind": "SavedModel" if graph.get("likely_savedmodel") else "GraphDef",
                "node_count": graph.get("node_count"),
                "distinct_ops": graph.get("distinct_ops"),
                "likely_savedmodel": graph.get("likely_savedmodel"),
            }, "graphdef", local_fid)
            for op, n in graph.get("top_ops", []):
                self._add_property(ds["dataset_id"], f"op.{op}", n,
                                   "graphdef_ops", local_fid)
    ...
    def _handle_pbtxt(self, ds, data, local_fid):
        ...
        for name, n in sorted(block_names.items()):
            self._add_property(ds["dataset_id"], f"block.{name}", n,
                               "textproto_blocks", local_fid)
    ...
    def _handle_image(self, path, ext, category, subcategory, local_fid):
        ds = self._add_dataset(path, ext, category or "image",
                               subcategory or "raster_common", "image", local_fid)
        try:
        ...
        if base in (".tif", ".tiff"):
            try:
                ...
                if geo:
                    geo["driver"] = "geotiff"
                    self._add_properties(ds["dataset_id"], geo,
                                         "geospatial", local_fid)
    ...
    def _handle_wav(self, path, ext, category, subcategory, local_fid):
        import wave
        ds = self._add_dataset(path, ext, category or "audio",
                               subcategory or "uncompressed_pcm", "audio", local_fid)
        try:
    ...
    def _handle_audio(self, path, ext, category, subcategory, local_fid):
        ...
        ds = self._add_dataset(path, ext, category or "audio",
                               subcategory or "compressed_lossy", "audio", local_fid)
        ...
        try:
            ...
            if mf is not None and getattr(mf, "info", None) is not None:
                ...
                props = {"probe": "mutagen"}
                for src, dst in (("sample_rate", "sample_rate_hz"),
                                 ("channels", "channels"),
                                 ("bitrate", "bitrate_bps"),
        ...
        if props:
            ...
            ds["analysis_status"] = "partial"
            ds["notes"] = "audio classified; no header parser for this codec " \
    ...
    def _audio_header_probe(path, ext):
        ...
        from .science_formats import (read_caf, read_ogg, read_wavpack,
                                       read_tta, read_ape)
        _exotic = {b"caff": read_caf, b"OggS": read_ogg, b"wvpk": read_wavpack,
                   b"TTA1": read_tta, b"MAC ": read_ape}
        ...
        if head[:4] == b"fLaC":
            ...
            if len(info) >= 18:
                ...
                total = ((info[13] & 0x0F) << 32) | (info[14] << 24) | \
                        (info[15] << 16) | (info[16] << 8) | info[17]
                dur = (total / sr) if sr else None
                return {"codec": "flac", "sample_rate_hz": sr, "channels": ch,
                        "bit_depth": bps, "frame_count": total,
                        "duration_seconds": round(dur, 4) if dur else None}
            ...
            with open(path, "rb") as f:
                ...
                for _ in range(64):                   # scan chunk headers
                    ...
                    if cid == b"COMM":
                        ...
                        return {"codec": "aiff", "sample_rate_hz": sr,
                                "channels": nch, "bit_depth": ssize,
                                "frame_count": nf,
                                "duration_seconds": round(dur, 4) if dur else None}
        ...
        if head[:4] == b".snd":
            ...
            if len(hdr) >= 24:
                ...
                return {"codec": "au", "sample_rate_hz": sr, "channels": ch,
                        "encoding_id": enc}
        ...
        _SR = {0: [44100,48000,32000], 3: [44100,48000,32000],
               2: [22050,24000,16000], 1: [11025,12000,8000]}
        while i != -1 and i + 4 <= len(buf):
            if buf[i] == 0xFF and (buf[i+1] & 0xE0) == 0xE0:
                ...
                if br_i not in (0, 15) and sr_i != 3 and ver in _SR:
                    sr = _SR[ver][sr_i]
                    br = _BR[1][br_i] * 1000
                    return {"codec": "mp3" if layer == 1 else "mpeg_audio",
                            "sample_rate_hz": sr, "bitrate_bps": br,
    ...
    def _handle_video(self, path, ext, category, subcategory, local_fid):
        ds = self._add_dataset(path, ext, category or "video",
                               subcategory or "containers_common", "video", local_fid)
        try:
            import cv2
        except Exception as err:
            ds["analysis_status"] = "unavailable"
    ...
    def _handle_pdf(self, path, ext, category, subcategory, local_fid):
        ds = self._add_dataset(path, ext, category or "text",
    ...
    def _handle_pickle(self, path, ext, category, subcategory, local_fid):
        ...
        for gname, cnt in list(sorted(globals_seen.items(),
                                      key=lambda kv: -kv[1]))[:50]:
            self._add_property(ds["dataset_id"], f"global::{gname}", cnt,
                               "pickle_globals", local_fid)
    ...
    def _handle_generic(self, path, ext, category, subcategory, local_fid):
        ...
        magic = self._read_magic(path, 16)
        if magic:
            self._add_property(ds["dataset_id"], "magic_hex", magic.hex(),
                               "binary", local_fid)
    ...
    def _handle_catalog_text(self, path, ext, category, subcategory, local_fid):
        ...
        ds = self._add_dataset(
            path, ext, category or "data", result["subcategory"],
            result["modality"], local_fid, status=result["status"],
        ...
        if cols:
            ...
            for i, col in enumerate(cols):
                self._add_column(
                    ds["dataset_id"], str(col.get("name", f"col{i}")), i,
                    str(col.get("dtype", "unknown")), "declared",
                    None, None, None, None, None, None, None, [], {}, local_fid)
    ...
    def _handle_catalog_assets(self, path, ext, category, subcategory, local_fid):
        ...
        if result.get("record_count") is not None:
            ds["record_count"] = result["record_count"]
        self._add_properties(ds["dataset_id"], result["props"],
                             result["family"], local_fid)
        cols = result.get("columns")
        if cols:
            ds["column_count"] = len(cols)
            for i, col in enumerate(cols):
        ...
        if tensors:
            ...
            for t in tensors:
                self._add_tensor(
                    ds["dataset_id"], str(t.get("name", "tensor")),
                    t.get("dtype"), t.get("shape") or [], t.get("num_bytes"),
                    t.get("extra") or {}, local_fid)
        layers = result.get("model_layers")
        if layers:
            for lyr in layers:
                self._add_model_layer(
                    ds["dataset_id"], str(lyr.get("layer_name", "layer")),
                    str(lyr.get("layer_type", "layer")),
                    int(lyr.get("ordinal", 0)), int(lyr.get("depth", 0)),
                    int(lyr.get("tensor_count", 0)),
                    int(lyr.get("param_tensor_count", 0)),
                    int(lyr.get("buffer_tensor_count", 0)),
                    lyr.get("total_parameters"),
                    lyr.get("total_buffer_elements"), lyr.get("total_bytes"),
                    lyr.get("dtypes") or [], lyr.get("param_shapes") or {},
                    lyr.get("roles") or {}, local_fid)
    ...
    def _handle_georaster(self, path, ext, category, subcategory, local_fid):
        ...
        base = ("." + ext.split(".")[-1]) if ext else ext
        ds = self._add_dataset(path, ext, category or "geospatial",
        ...
        try:
            ...
            if g is not None:
                info = {"driver": g.GetDriver().ShortName,
                        "width": g.RasterXSize, "height": g.RasterYSize,
        ...
        try:
            ...
            if m:
                self._add_property(ds["dataset_id"], "magic_hex", m.hex(),
                                   "binary", local_fid)
    ...
    def _emit_raster(self, ds, info, local_fid):
        ...
        if w and h:
            ...
            self._add_tensor(ds["dataset_id"], "raster",
                             info.get("data_type"), shape, None,
                             {"driver": info.get("driver")}, local_fid)
            ds["tensor_count"] = 1
            ds["record_count"] = w * h
    ...
    def _emit_netcdf(self, ds, info, local_fid):
        ...
        for d in info.get("dimensions", [])[:200]:
            self._add_property(ds["dataset_id"], "dim::" + str(d["name"]),
                               d["size"], "netcdf_dimensions", local_fid)
        n = 0
        for v in info.get("variables", []):
            self._add_tensor(ds["dataset_id"], v["name"], v.get("dtype"),
                             v.get("shape", []), None,
                             {"dims": v.get("dims", [])}, local_fid)
            n += 1
        ds["tensor_count"] = n
    ...
    def _handle_medical(self, path, ext, category, subcategory, local_fid):
        ...
        if not shape and info.get("rows") and info.get("columns"):
            ...
            shape = ([frames, info["rows"], info["columns"]] if frames and frames > 1
                     else [info["rows"], info["columns"]])
        if shape:
            self._add_tensor(ds["dataset_id"], "volume", info.get("dtype"),
                             shape, None, {"modality": info.get("modality")},
                             local_fid)
            ds["tensor_count"] = 1
            n = 1
            for d in shape:
    ...
    def _medical_dicom(self, path, sf):
        try:
            ...
            for attr, key in (("Modality", "modality"),
                ...
                if v is not None:
                    info[key] = int(v) if key in (
                        "rows", "columns", "bits_allocated",
                        "pixel_representation", "samples_per_pixel",
                        "number_of_frames") else str(v)[:128]
    ...
    def _handle_camera_raw(self, path, ext, category, subcategory, local_fid):
        ...
        from . import science_formats as sf
        ds = self._add_dataset(path, ext, category or "image",
                               subcategory or "camera_raw", "image", local_fid)
        # optional rawpy fast path (exact sensor dimensions)
        try:
            import rawpy
            with rawpy.imread(str(path)) as r:
                s = r.sizes
                info = {"width": s.raw_width, "height": s.raw_height,
                        "processed_width": s.width, "processed_height": s.height,
                        "colors": r.num_colors, "probe": "rawpy"}
                self._add_properties(ds["dataset_id"], info, "camera_raw",
                                     local_fid)
                self._add_tensor(ds["dataset_id"], "sensor", "uint16",
                                 [s.raw_height, s.raw_width], None,
                                 {"cfa": True}, local_fid)
                ds["tensor_count"] = 1
        ...
        w, h = info.get("width"), info.get("height")
        if w and h:
            self._add_tensor(ds["dataset_id"], "ifd0_image",
                             None, [h, w], None,
    ...
    def _handle_pointcloud(self, path, ext, category, subcategory, local_fid):
        ...
        from . import science_formats as sf
        base = ("." + ext.split(".")[-1]) if ext else ext
        ds = self._add_dataset(path, ext, category or "geometry",
                               subcategory or "point_cloud", "geometry", local_fid)
        try:
            if base == ".pcd":
                info = sf.read_pcd(path)
                self._emit_geometry(
                    ds, local_fid, points=info.get("point_count"),
                    attributes=info.get("fields"),
                    props={k: v for k, v in info.items()
                           if k in ("format", "encoding", "pcd_version",
                                    "width", "height", "field_types",
                                    "field_count")})
                return
            if base in (".las", ".laz"):
        ...
        props = {k: v for k, v in info.items()
                 if k in ("format", "las_version", "e57_version", "point_format",
                          "compressed", "scale", "offset", "scan_count",
                          "point_record_length", "crs", "probe")}
        self._emit_geometry(ds, local_fid, points=info.get("point_count"),
    ...
    def _emit_geometry(self, ds, local_fid, *, vertices=None, faces=None,
                       points=None, edges=None, cells=None, bbox=None,
                       attributes=None, props=None):
        primary = next((v for v in (vertices, points, cells, faces)
    ...
    def _emit_geo(self, ds, local_fid, *, features=None, geometries=None,
                  crs=None, bbox=None, geom_types=None, props=None):
    ...
    def _emit_graph(self, ds, local_fid, *, nodes=None, edges=None,
                    directed=None, attributes=None, props=None):
    ...
    def _emit_config(self, ds, local_fid, *, keys=None, sections=None,
                     section_names=None, fmt=None, props=None):
        if keys is not None:
    ...
    def _handle_mesh(self, path, ext, category, subcategory, local_fid):
        base = ("." + ext.split(".")[-1]) if ext else ext
        ds = self._add_dataset(path, ext, category or "geometry",
                               subcategory or "mesh_3d", "geometry", local_fid)
        parser = {
            ".obj": self._mesh_obj, ".off": self._mesh_off, ".ply": self._mesh_ply,
            ".stl": self._mesh_stl, ".xyz": self._mesh_pointcloud,
            ".pts": self._mesh_pointcloud, ".gltf": self._mesh_gltf,
            ".msh": self._mesh_gmsh, ".vtu": self._mesh_vtk_xml,
            ".vtp": self._mesh_vtk_xml, ".vti": self._mesh_vtk_xml,
            ".dae": self._mesh_collada, ".x3d": self._mesh_x3d,
            ".usda": self._mesh_usd, ".mtl": self._mesh_mtl,
        }.get(base)
        if parser is None:
            ds["analysis_status"] = "generic"
            return
    ...
    def _mesh_obj(self, path, ds, local_fid):
        ...
        with open(path, "r", encoding="utf-8", errors="replace") as fh:
            for line in fh:
                ...
                if k == "v":
                    v += 1
                    try:
                        self._bbox_add(bb, float(t[1]), float(t[2]),
        ...
        self._emit_geometry(ds, local_fid, vertices=v, faces=f, bbox=bb,
                            attributes=attrs or None,
                            props={"normal_count": vn or None,
        ...
        with open(path, "r", encoding="utf-8", errors="replace") as fh:
            ...
            if nverts:
                for _ in range(min(nverts, self.MAX_GEOM_VERTS)):
                    ...
                    c = vl.split()
                    try:
                        self._bbox_add(bb, float(c[0]), float(c[1]),
                                       float(c[2]) if len(c) > 2 else None)
                    except (ValueError, IndexError):
                        pass
        self._emit_geometry(ds, local_fid, vertices=nverts, faces=nfaces,
                            edges=nedges or None, bbox=bb)
            ...
            if fmt == "ascii" and vcount:
                ...
                for _ in range(min(vcount, self.MAX_GEOM_VERTS)):
                    ...
                    try:
                        ...
                        pass
        self._emit_geometry(ds, local_fid, vertices=vcount, faces=fcount,
                            edges=ecount or None, bbox=bb,
                            attributes=vprops or None,
                            props={"ply_format": fmt,
                                   "elements": {n: c for n, c in elements}})
                ...
                for line in fh:
                    ...
                    if s.startswith("facet"):
                        ...
                        try:
                            ...
                            pass
            self._emit_geometry(ds, local_fid, vertices=verts, faces=facets,
                                bbox=bb, props={"stl_format": "ascii",
                                                "scan_truncated": trunc})
        else:
            with open(path, "rb") as fh:
                fh.seek(80)
                ...
                for _ in range(min(ntri, self.MAX_GEOM_VERTS)):
                    ...
                    for kk in range(3):
                        self._bbox_add(bb, vals[3 + 3 * kk], vals[4 + 3 * kk],
                                       vals[5 + 3 * kk])
            self._emit_geometry(ds, local_fid, vertices=ntri * 3, faces=ntri,
                                bbox=bb, props={"stl_format": "binary"})
    ...
    def _mesh_pointcloud(self, path, ds, local_fid):
        ...
        with open(path, "r", encoding="utf-8", errors="replace") as fh:
            for line in fh:
                ...
                if len(c) > maxcols:
                    maxcols = len(c)
        self._emit_geometry(ds, local_fid, points=pts, bbox=bb,
                            props={"columns_per_point": maxcols or None,
                                   "has_color": maxcols >= 6,
                                   "scan_truncated": trunc})
    ...
    def _mesh_gltf(self, path, ds, local_fid):
        ...
        self._emit_geometry(ds, local_fid, vertices=vcount or None, bbox=bb,
                            attributes=sorted(attr_names) or None,
                            props={"mesh_count": len(meshes),
            ...
            for line in it:
                ...
                if s == "$MeshFormat":
                    ...
                    try:
                        ...
                        pass
        self._emit_geometry(ds, local_fid, vertices=nnodes, cells=nelems,
    ...
    def _mesh_vtk_xml(self, path, ds, local_fid):
        ...
        self._emit_geometry(ds, local_fid, points=points, cells=cells,
                            vertices=verts, attributes=attrs or None,
                            props={"vtk_type": m_type.group(1) if m_type else None,
                                   "poly_count": polys, "line_count": lines,
                                   "strip_count": strips,
    ...
    def _mesh_collada(self, path, ds, local_fid):
        ...
        floatcounts = [int(x) for x in
                       re.findall(r'<float_array[^>]*\bcount="(\d+)"', text)]
        faces = sum(int(x) for x in (
            re.findall(r'<triangles[^>]*\bcount="(\d+)"', text)
            + re.findall(r'<polylist[^>]*\bcount="(\d+)"', text)
            + re.findall(r'<polygons[^>]*\bcount="(\d+)"', text))) or None
        ...
        self._emit_geometry(ds, local_fid, vertices=verts, faces=faces,
                            props={"geometry_count": geoms,
                                   "vertex_count_estimated": True,
                                   "float_array_max": max(floatcounts)
                                   if floatcounts else None})
    ...
    def _mesh_x3d(self, path, ds, local_fid):
        ...
        for pa in coords_attrs:
            ...
            for i in range(0, len(nums) - 2, 3):
                try:
                    self._bbox_add(bb, float(nums[i]), float(nums[i + 1]),
                                   float(nums[i + 2]))
                    vcount += 1
                except ValueError:
                    pass
        self._emit_geometry(ds, local_fid, vertices=vcount or None, bbox=bb,
                            props={"indexed_face_set_count": ifs,
    ...
    def _mesh_usd(self, path, ds, local_fid):
        ...
        self._emit_geometry(ds, local_fid, vertices=vcount,
                            props={"prim_count": len(prims),
    ...
    def _mesh_mtl(self, path, ds, local_fid):
        ...
        self._add_properties(ds["dataset_id"],
                             {"material_count": materials,
                              "texture_map_count": maps or None},
                             "geometry", local_fid)
    ...
    def _handle_geospatial(self, path, ext, category, subcategory, local_fid):
        base = ("." + ext.split(".")[-1]) if ext else ext
        ds = self._add_dataset(path, ext, category or "geospatial",
                               subcategory or "vector_geo", "geospatial", local_fid)
        try:
    ...
    def _geo_parse(self, path, ds, base, local_fid):
        fn = {".geojson": self._geo_geojson, ".topojson": self._geo_topojson,
              ".kml": self._geo_kml, ".gpx": self._geo_gpx, ".osm": self._geo_osm,
              ".wkt": self._geo_wkt, ".prj": self._geo_prj,
              ".gml": self._geo_gml}.get(base)
        if fn is None:
            ds["analysis_status"] = "generic"
            return
    ...
    def _geo_geojson(self, path, ds, local_fid):
        ...
        bbox = docbbox if isinstance(docbbox, list) else (
            bb if any(v is not None for v in bb) else None)
        self._emit_geo(ds, local_fid, features=features,
                       geometries=sum(gt.values()) or None, crs=crs, bbox=bbox,
    ...
    def _geo_topojson(self, path, ds, local_fid):
        ...
        for _name, obj in objects.items():
            ...
            if t == "GeometryCollection":
                ...
                gt[t] += 1
        bbox = doc.get("bbox")
        self._emit_geo(ds, local_fid, features=feat or None,
                       geometries=sum(gt.values()) or None,
                       bbox=bbox if isinstance(bbox, list) else None,
                       geom_types=dict(gt) or None,
                       props={"object_count": len(objects),
                              "arc_count": len(arcs),
    ...
    def _geo_kml(self, path, ds, local_fid):
        ...
        gt = Counter()
        for g in ("Point", "LineString", "Polygon", "MultiGeometry",
                  "LinearRing", "Model", "gx:Track"):
            ...
            for tok in coord.replace("\n", " ").split():
                ...
                if len(parts) >= 2:
                    try:
                        ...
                        pass
        self._emit_geo(ds, local_fid, features=placemarks or None,
                       geometries=sum(gt.values()) or None, crs="EPSG:4326",
    ...
    def _geo_gpx(self, path, ds, local_fid):
        ...
        self._emit_geo(ds, local_fid, features=total or None, crs="EPSG:4326",
                       bbox=bb if any(v is not None for v in bb) else None,
                       geom_types={"trkpt": trkpts, "wpt": wpts, "rtept": rtepts},
                       props={"track_count": tracks, "route_count": routes,
                              "waypoint_count": wpts, "trackpoint_count": trkpts})
        ...
        if not any(v is not None for v in bb):
            for lat, lon in re.findall(
                ...
                try:
                    ...
                    pass
        self._emit_geo(ds, local_fid, features=(nodes + ways + rels) or None,
                       crs="EPSG:4326",
                       bbox=bb if any(v is not None for v in bb) else None,
                       geom_types={"node": nodes, "way": ways, "relation": rels},
                       props={"node_count": nodes, "way_count": ways,
                              "relation_count": rels})
            ...
            for line in fh:
                ...
                for x, y in re.findall(r'(-?\d+\.?\d*)\s+(-?\d+\.?\d*)', s):
                    try:
                        ...
                        pass
        self._emit_geo(ds, local_fid, geometries=sum(gt.values()) or None, crs=crs,
                       bbox=bb if any(v is not None for v in bb) else None,
                       geom_types=dict(gt) or None,
    ...
    def _geo_prj(self, path, ds, local_fid):
        ...
        self._emit_geo(ds, local_fid, crs=(epsg or projcs or geogcs),
                       props={"projected_cs": projcs, "geographic_cs": geogcs,
    ...
    def _geo_gml(self, path, ds, local_fid):
        ...
        gt = Counter()
        for g in ("Point", "LineString", "Polygon", "MultiSurface", "MultiCurve",
                  "Surface", "Curve", "LinearRing", "MultiPolygon"):
        ...
        for txt in re.findall(
            ...
            for i in range(0, len(nums) - 1, 2):
                try:
                    ...
                    pass
        self._emit_geo(ds, local_fid, features=feats or None,
                       geometries=sum(gt.values()) or None,
    ...
    def _handle_gml_dispatch(self, path, ext, category, subcategory, local_fid):
        ...
        is_graph = (not is_xml and "graph" in low and "[" in low
                    and ("node" in low or "edge" in low))
        if is_graph:
            ds = self._add_dataset(path, ext, category or "graph",
                                   "graph_modeling_language", "graph", local_fid)
            try:
            ...
            ds = self._add_dataset(path, ext, category or "geospatial",
                                   "geography_markup", "geospatial", local_fid)
            try:
    ...
    def _handle_graph(self, path, ext, category, subcategory, local_fid):
        base = ("." + ext.split(".")[-1]) if ext else ext
        ds = self._add_dataset(path, ext, category or "graph",
                               subcategory or "graph_network", "graph", local_fid)
        try:
    ...
    def _graph_parse(self, path, ds, base, local_fid):
        fn = {".graphml": self._graph_graphml, ".gexf": self._graph_gexf,
              ".dot": self._graph_dot, ".gv": self._graph_dot,
              ".net": self._graph_pajek, ".edgelist": self._graph_edgelist,
              ".mtx": self._graph_matrixmarket, ".mm": self._graph_matrixmarket,
              ".gml": self._graph_gml}.get(base)
        if fn is None:
            ds["analysis_status"] = "generic"
            return
        fn(path, ds, local_fid)
    ...
    def _graph_graphml(self, path, ds, local_fid):
        ...
        self._emit_graph(ds, local_fid, nodes=nodes, edges=edges, directed=directed,
    ...
    def _graph_dot(self, path, ds, local_fid):
        ...
        node_ids = set()
        for m in re.finditer(r'([A-Za-z0-9_."]+)\s*(?:->|--)\s*([A-Za-z0-9_."]+)',
                             text):
            node_ids.add(m.group(1))
            node_ids.add(m.group(2))
        for m in re.finditer(r'(?m)^\s*("?[\w.]+"?)\s*\[', text):
            node_ids.add(m.group(1))
        self._emit_graph(ds, local_fid, nodes=len(node_ids) or None,
                         edges=edges or None, directed=directed,
            ...
            for line in fh:
                ...
                if s.startswith("*"):
                    ...
                    if m:
                        nodes = int(m.group(1))
                        section = "vertices"
                    elif (low.startswith("*edges") or low.startswith("*arcs")
                ...
                if section == "edges":
                    edge_ct += 1
        self._emit_graph(ds, local_fid, nodes=nodes, edges=edge_ct or None,
                         props={"format": "pajek"})
    ...
    def _graph_edgelist(self, path, ds, local_fid):
        ...
        with open(path, "r", encoding="utf-8", errors="replace") as fh:
            for line in fh:
                ...
                if len(parts) >= 3:
                    weighted = True
        self._emit_graph(ds, local_fid, nodes=len(nodes) or None, edges=edges,
    ...
    def _graph_matrixmarket(self, path, ds, local_fid):
        ...
        self._emit_graph(ds, local_fid, nodes=nodes, edges=nnz,
                         props={"matrix_rows": rows, "matrix_cols": cols,
                                "nonzeros": nnz, "mm_format": fmt,
                                "mm_symmetry": symmetry, "banner": banner})
    ...
    def _graph_gml(self, path, ds, local_fid):
        ...
        self._emit_graph(ds, local_fid, nodes=nodes or None, edges=edges or None,
                         directed=directed,
    ...
    def _handle_config(self, path, ext, category, subcategory, local_fid):
        base = ("." + ext.split(".")[-1]) if ext else ext
        ds = self._add_dataset(path, ext, category or "config",
                               subcategory or "configuration", "config", local_fid)
        try:
            if base == ".toml":
                ...
                self._config_ini_like(path, ds, local_fid, fmt={
                    ".ini": "ini", ".cfg": "ini", ".conf": "conf",
                    ".properties": "properties"}.get(base, "ini"))
        except Exception as err:
            ds["analysis_status"] = "partial"
    ...
    def _config_toml(self, path, ds, local_fid):
        ...
        if tomllib is not None:
            ...
            nk, sects = count(data)
            self._emit_config(ds, local_fid, keys=nk, sections=len(sects) or None,
                              section_names=sects or None, fmt="toml")
            return
        self._config_ini_like(path, ds, local_fid, fmt="toml")
    ...
    def _config_ini_like(self, path, ds, local_fid, fmt="ini"):
        ...
        with open(path, "r", encoding="utf-8", errors="replace") as fh:
            for line in fh:
                ...
                if "=" in s or ":" in s:
                    keys += 1
        self._emit_config(ds, local_fid, keys=keys,
                          sections=len(sections) or None,
                          section_names=sections or None, fmt=fmt,
                          props={"scan_truncated": trunc})
    ...
    def _config_env(self, path, ds, local_fid):
        ...
        self._emit_config(ds, local_fid, keys=keys, fmt="dotenv",
                          props={"variable_names": names or None,
                                 "scan_truncated": trunc})
    ...
    def _config_hcl(self, path, ds, local_fid):
        ...
        self._emit_config(ds, local_fid, keys=len(assigns) or None,
                          sections=len(blocks) or None,
                          section_names=blocks or None, fmt="hcl",
                          props={"block_types": dict(Counter(blocks)) or None,
    ...
    def _config_jsonnet(self, path, ds, local_fid):
        ...
        for raw in text.splitlines():
            ...
            if depth < 0:
                depth = 0
        self._emit_config(ds, local_fid, keys=fields or None, fmt="jsonnet",
                          props={"local_count": locals_,
    ...
    def _handle_bio(self, path, ext, category, subcategory, local_fid):
        base = ("." + ext.split(".")[-1]) if ext else ext
        ds = self._add_dataset(path, ext, category or "bioinformatics",
                               subcategory or "sequence_data",
                               "bioinformatics", local_fid)
        try:
    ...
    def _bio_fasta(self, path, ds, local_fid):
        ...
        with open(path, "r", encoding="utf-8", errors="replace") as fh:
            ...
            if cur:
                longest = max(longest, cur)
        self._emit_bio(ds, local_fid, sequences=seqs,
                       props={"total_residues": total,
        ...
        with open(path, "r", encoding="utf-8", errors="replace") as fh:
            for line in fh:
                ...
                if scanned > self.MAX_STRUCT_SCAN_BYTES:
                    ...
                    break
        reads = lines // 4
        self._emit_bio(ds, local_fid, records=reads,
                       props={"line_count": lines, "reads": reads,
                              "scan_truncated": trunc})
        ...
        with open(path, "r", encoding="utf-8", errors="replace") as fh:
            for line in fh:
                ...
                if line.startswith("@"):
                    ...
                    aln += 1
        self._emit_bio(ds, local_fid, records=aln,
                       props={"alignment_count": aln, "reference_count": refs,
                              "header_lines": header, "sam_version": ver,
                              "scan_truncated": trunc})
        ...
        with open(path, "r", encoding="utf-8", errors="replace") as fh:
            for line in fh:
                ...
                if line.startswith("##"):
                    ...
                    variants += 1
        self._emit_bio(ds, local_fid, records=variants,
                       props={"variant_count": variants, "sample_count": samples,
                              "meta_lines": meta, "vcf_version": ver,
                              "scan_truncated": trunc})
    ...
    def _bio_bed(self, path, ds, local_fid):
        ...
        with open(path, "r", encoding="utf-8", errors="replace") as fh:
            for line in fh:
                ...
                if nc > maxcols:
                    maxcols = nc
        self._emit_bio(ds, local_fid, records=feats,
                       props={"feature_count": feats, "bed_columns": maxcols or None,
                              "scan_truncated": trunc})
    ...
    def _bio_mol(self, path, ds, local_fid):
        ...
        if len(lines) >= 4 and lines[3]:
            ...
            try:
                ...
                pass
        self._emit_bio(ds, local_fid, records=1,
                       props={"molecule_count": 1, "atom_count": atoms,
                              "bond_count": bonds, "title": name})
            ...
            for line in fh:
                ...
                if mols == 0 and line_in_mol == 4 and first_atoms is None:
                    try:
                        ...
                        pass
        self._emit_bio(ds, local_fid, records=mols or None,
                       props={"molecule_count": mols,
        ...
        with open(path, "r", encoding="utf-8", errors="replace") as fh:
            for line in fh:
                ...
                if rec == "ATOM":
                    ...
                    title = line[10:].strip()
        self._emit_bio(ds, local_fid, records=atoms,
                       props={"atom_count": atoms, "hetatm_count": hetatm,
                              "model_count": models or None,
                              "chain_count": len(chains) or None,
                              "seqres_lines": seqres or None,
                              "title": title or None, "scan_truncated": trunc})
