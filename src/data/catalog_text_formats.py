...
def _read_text(path: Path, cap: int = _SCAN_CAP) -> Tuple[str, str, bool]:
    ...
    for enc in ("utf-8-sig", "utf-8"):
        try:
            return raw.decode(enc), ("utf-8-bom" if enc == "utf-8-sig"
                                     and raw[:3] == b"\xef\xbb\xbf" else "utf-8"), truncated
...
def generic_text_metrics(text: str) -> Dict[str, Any]:
    ...
    return {"line_count": len(lines), "non_empty_lines": non_empty,
            "word_count": words, "char_count": len(text),
            "max_line_length": max((len(ln) for ln in lines), default=0)}


def _ok(family: str, modality: str, subcategory: str, props: Dict[str, Any],
        record_count: Optional[int] = None, columns: Optional[List[Dict]] = None,
        status: str = "ok") -> Dict[str, Any]:
    return {"family": family, "modality": modality, "subcategory": subcategory,
            "record_count": record_count, "columns": columns, "status": status,
...
def parse_subtitle(text: str, ext: str) -> Dict[str, Any]:
    ...
    props = {"format": fmt, "cue_count": cues,
...
def parse_playlist(text: str, ext: str) -> Dict[str, Any]:
    if ext in (".m3u", ".m3u8"):
        ...
        return _ok("playlist", "text", "media_playlist",
                   {"format": "M3U", "entry_count": len(entries),
                    "extinf_tags": extinf}, record_count=len(entries))
    if ext == ".pls":
        n = len(re.findall(r"(?mi)^File\d+=", text))
        return _ok("playlist", "text", "media_playlist",
                   {"format": "PLS", "entry_count": n}, record_count=n)
    if ext in (".xspf", ".asx", ".wpl", ".b4s", ".wax"):
        n = len(re.findall(r"(?i)<(track|entry|ref|media)[ >]", text))
        return _ok("playlist", "text", "media_playlist",
                   {"format": "XML playlist", "entry_count": n}, record_count=n)
    if ext in (".cue", ".toc"):
        tracks = len(re.findall(r"(?mi)^\s*TRACK\s+\d+", text))
        return _ok("playlist", "text", "cue_sheet",
                   {"format": "CUE/TOC", "track_count": tracks}, record_count=tracks)
    # ffp / md5 checksum lists
    lines = [l for l in text.splitlines() if l.strip()]
    return _ok("playlist", "text", "checksum_list",
...
def parse_rdf(text: str, ext: str) -> Dict[str, Any]:
    if ext in (".jsonld",):
        try:
            ...
            return _ok("rdf", "text", "linked_data",
                       {"format": "JSON-LD", "node_count": n,
    ...
    for ln in text.splitlines():
        s = ln.strip()
        if s.endswith(".") and not s.lower().startswith(("@prefix", "@base",
                                                         "prefix ", "base ")):
            triples += 1
    semis = text.count(";")
    return _ok("rdf", "text", "rdf_graph",
               {"format": "Turtle/N-Triples", "prefix_count": len(prefixes),
                "prefixes": prefixes[:32] or None,
                "approx_triples": triples, "predicate_object_lists": semis},
...
def parse_sparql(text: str, ext: str) -> Dict[str, Any]:
    if ext == ".srx" or (ext == ".xml" and "<sparql" in text[:512]):
        ...
        return _ok("sparql", "text", "sparql_results",
    ...
    return _ok("sparql", "text", "sparql_query",
               {"format": "SPARQL", "query_form": form,
                "variable_count": len(variables), "variables": variables[:32] or None,
...
def parse_icalendar(text: str, ext: str) -> Dict[str, Any]:
    if re.search(r"(?mi)^BEGIN:VCARD", text):
        ...
        return _ok("icalendar", "text", "vcard",
                   {"format": "vCard", "card_count": n}, record_count=n)
    comps = {}
    ...
    return _ok("icalendar", "text", "icalendar",
               {"format": "iCalendar", "components": comps or None,
                "component_count": total}, record_count=total)
...
def parse_email(text: str, ext: str) -> Dict[str, Any]:
    if ext in (".mbox",) or re.match(r"From \S+@?\S* ", text):
        ...
        return _ok("email", "text", "mailbox",
                   {"format": "mbox", "message_count": msgs}, record_count=msgs)
    headers = len(re.findall(r"(?m)^[A-Za-z-]+:\s", text.split("\n\n", 1)[0]))
    subject = re.search(r"(?mi)^Subject:\s*(.+)$", text)
    return _ok("email", "text", "email_message",
               {"format": "RFC-822 message", "header_count": headers,
                "has_subject": bool(subject)}, record_count=1)
...
def parse_ldif(text: str, ext: str) -> Dict[str, Any]:
    ...
    return _ok("ldif", "text", "ldap_directory",
               {"format": "LDIF", "entry_count": entries,
                "changetype_count": changetypes}, record_count=entries)
...
def parse_notebook(text: str, ext: str) -> Dict[str, Any]:
    if ext in (".ipynb", ".zpln"):
        try:
            ...
            return _ok("notebook", "text", "computational_notebook",
                       {"format": "Jupyter/Zeppelin", "cell_count": len(cells),
                        "cell_types": kinds or None, "language": lang},
    ...
    return _ok("notebook", "text", "computational_notebook",
               {"format": "Markdown notebook", "code_chunk_count": chunks,
                "section_count": headers}, record_count=chunks)
...
def parse_json_tolerant(text: str, ext: str) -> Dict[str, Any]:
    ...
    if isinstance(obj, dict):
        props = {"format": "JSON object", "top_level_keys": len(obj),
...
def parse_xml(text: str, ext: str) -> Dict[str, Any]:
    ...
    props = {"format": "XML", "root_element": root,
             "element_count": len(tags), "distinct_elements": len(hist),
             "top_elements": dict(hist.most_common(12)) or None,
             "namespace_count": len(namespaces),
             "namespaces": namespaces[:16] or None, "max_depth": maxdepth}
...
def parse_html(text: str, ext: str) -> Dict[str, Any]:
    ...
    links = hist.get("a", 0)
    return _ok("htmlfam", "text", "html_document",
               {"format": "HTML", "tag_count": len(tags),
                "distinct_tags": len(hist), "top_tags": dict(hist.most_common(12)),
                "link_count": links, "has_title": bool(title)},
...
def parse_css(text: str, ext: str) -> Dict[str, Any]:
    ...
    return _ok("cssfam", "text", "stylesheet",
               {"format": ext.lstrip(".").upper() or "CSS", "rule_count": rules,
                "selector_count": selectors, "at_rule_count": at_rules,
                "custom_property_count": variables}, record_count=rules)
...
def parse_yaml(text: str, ext: str) -> Dict[str, Any]:
    ...
    return _ok("yamlfam", "text", "yaml_document",
               {"format": "YAML", "document_count": docs,
                "top_level_keys": len(top_keys), "keys_sampled": top_keys[:32] or None,
                "list_item_count": list_items, "anchor_count": anchors},
               record_count=docs)
...
def parse_edi(text: str, ext: str) -> Dict[str, Any]:
    if ext in (".hl7", ".hl7v2") or text.startswith("MSH"):
        ...
        return _ok("edifam", "text", "hl7_message",
                   {"format": "HL7 v2", "message_count": msgs,
                    "segment_count": segs, "segment_types": seg_types[:32] or None},
                   record_count=msgs)
    if "UNB" in text[:64] or "UNH" in text[:256] or ext in (".edifact",):
        segs = text.count("'")
        msgs = text.count("UNH")
        return _ok("edifam", "text", "edifact_interchange",
                   {"format": "EDIFACT", "message_count": msgs,
                    "segment_count": segs}, record_count=msgs)
    if text.startswith("ISA") or ext in (".x12", ".edi", ".edi850", ".edi810",
                                          ".edi837", ".edi835", ".edi856"):
        st = len(re.findall(r"(?m)\bST\*", text))
        seg = text.count("~") or len(re.findall(r"\*", text))
        return _ok("edifam", "text", "x12_interchange",
                   {"format": "ANSI X12", "transaction_set_count": st,
                    "segment_count": seg}, record_count=st)
    # ACH/NACHA fixed-width
    lines = [l for l in text.splitlines() if l.strip()]
    return _ok("edifam", "text", "edi_record",
...
def parse_bio_text(text: str, ext: str) -> Dict[str, Any]:
    if ext in (".gb", ".gbk", ".genbank"):
        n = text.count("LOCUS")
        return _ok("biotext", "scientific", "genbank",
                   {"format": "GenBank", "record_count": n}, record_count=n)
    if ext in (".embl",):
        n = len(re.findall(r"(?m)^ID   ", text))
        return _ok("biotext", "scientific", "embl",
                   {"format": "EMBL", "record_count": n}, record_count=n)
    if ext in (".nwk", ".newick", ".tree"):
        trees = text.count(";")
        leaves = len(re.findall(r"[(,]\s*([A-Za-z0-9_.]+)\s*[:,)]", text))
        return _ok("biotext", "scientific", "phylo_tree",
                   {"format": "Newick", "tree_count": trees, "leaf_count": leaves},
                   record_count=trees)
    if ext in (".nex", ".nexus"):
        blocks = len(re.findall(r"(?i)begin\s+\w+;", text))
        return _ok("biotext", "scientific", "nexus",
                   {"format": "NEXUS", "block_count": blocks}, record_count=blocks)
    if ext in (".phy", ".phylip", ".aln", ".sto", ".stockholm"):
        seqs = len([l for l in text.splitlines() if l.strip() and not l.startswith(("#", ">"))])
        return _ok("biotext", "scientific", "alignment",
                   {"format": "alignment", "line_count": seqs}, record_count=seqs)
    if ext in (".mgf",):
        n = text.count("BEGIN IONS")
        return _ok("biotext", "scientific", "mass_spec_peaklist",
                   {"format": "MGF", "spectrum_count": n}, record_count=n)
    if ext in (".wig", ".bedgraph"):
        n = len([l for l in text.splitlines() if l.strip() and l[0].isdigit()])
        return _ok("biotext", "scientific", "genome_track",
                   {"format": "WIG", "data_line_count": n}, record_count=n)
    if ext in (".ped", ".fam", ".bim", ".map"):
        lines = [l for l in text.splitlines() if l.strip()]
        return _ok("biotext", "scientific", "plink_genotype",
                   {"format": "PLINK", "record_count": len(lines)},
                   record_count=len(lines))
    if ext in (".gvcf",):
        variants = len([l for l in text.splitlines() if l and not l.startswith("#")])
        return _ok("biotext", "scientific", "gvcf",
                   {"format": "gVCF", "variant_count": variants}, record_count=variants)
    lines = [l for l in text.splitlines() if l.strip()]
    return _ok("biotext", "scientific", "bio_text",
               {"format": "bio text", "record_count": len(lines)}, record_count=len(lines))


def parse_molecule_text(text: str, ext: str) -> Dict[str, Any]:
    if ext in (".smi", ".smiles"):
        mols = [l for l in text.splitlines() if l.strip()]
        return _ok("moltext", "scientific", "smiles",
                   {"format": "SMILES", "molecule_count": len(mols)},
                   record_count=len(mols))
    if ext in (".inchi",):
        n = text.count("InChI=")
        return _ok("moltext", "scientific", "inchi",
                   {"format": "InChI", "molecule_count": n}, record_count=n)
    if ext in (".gro",):
        lines = text.splitlines()
        try:
            ...
            atoms = None
        return _ok("moltext", "scientific", "gromacs_structure",
                   {"format": "GROMACS GRO", "atom_count": atoms}, record_count=atoms)
    if ext in (".poscar", ".contcar", ".vasp"):
        ...
        if len(lines) > 6:
            try:
                ...
                pass
        return _ok("moltext", "scientific", "vasp_poscar",
                   {"format": "VASP POSCAR", "atom_count": counts}, record_count=counts)
    if ext in (".psf",):
        m = re.search(r"(\d+)\s*!NATOM", text)
        return _ok("moltext", "scientific", "psf_topology",
                   {"format": "CHARMM PSF",
                    "atom_count": int(m.group(1)) if m else None})
    if ext in (".xyz",):
        lines = text.splitlines()
        try:
        ...
        return _ok("moltext", "scientific", "xyz_geometry",
...
def parse_chess(text: str, ext: str) -> Dict[str, Any]:
    if ext == ".pgn":
        ...
        return _ok("chess", "text", "chess_pgn",
                   {"format": "PGN", "game_count": games}, record_count=games)
    if ext == ".sgf":
        games = text.count("(;")
        return _ok("chess", "text", "go_sgf",
                   {"format": "SGF", "game_tree_count": games}, record_count=games)
    positions = len([l for l in text.splitlines() if l.strip()])
    return _ok("chess", "text", "epd_positions",
               {"format": "EPD", "position_count": positions}, record_count=positions)


def parse_sensor_text(text: str, ext: str) -> Dict[str, Any]:
    if ext in (".nmea", ".gps", ".ais"):
        sentences = len(re.findall(r"(?m)^!?\$?[A-Z]{2}[A-Z]{3},", text)) or \
            len([l for l in text.splitlines() if l.startswith(("$", "!"))])
        types = sorted(set(re.findall(r"[$!](\w{5}),", text)))
        return _ok("sensortext", "scientific", "nmea_log",
                   {"format": "NMEA-0183", "sentence_count": sentences,
                    "sentence_types": types[:32] or None}, record_count=sentences)
    if ext in (".metar", ".taf"):
        obs = len([l for l in text.splitlines() if l.strip()])
        return _ok("sensortext", "scientific", "aviation_weather",
                   {"format": ext.lstrip(".").upper(), "observation_count": obs},
                   record_count=obs)
    if ext == ".igc":
        fixes = len(re.findall(r"(?m)^B\d", text))
        return _ok("sensortext", "scientific", "flight_track",
                   {"format": "IGC", "gps_fix_count": fixes}, record_count=fixes)
    lines = [l for l in text.splitlines() if l.strip()]
    return _ok("sensortext", "scientific", "sensor_log",
               {"format": "sensor text", "record_count": len(lines)}, record_count=len(lines))
...
def parse_gerber(text: str, ext: str) -> Dict[str, Any]:
    if ext in (".xln", ".drl", ".exc", ".nc", ".tap"):
        ...
        return _ok("gerber", "scientific", "excellon_drill",
    ...
    commands = text.count("*")
    return _ok("gerber", "scientific", "gerber_pcb",
               {"format": "Gerber RS-274X", "aperture_count": apertures,
                "operation_count": flashes, "command_count": commands},
...
def parse_math_model(text: str, ext: str) -> Dict[str, Any]:
    if ext in (".cnf", ".dimacs"):
        ...
        if m:
            return _ok("mathmodel", "scientific", "sat_cnf",
                       {"format": "DIMACS CNF", "variable_count": int(m.group(1)),
                        "clause_count": int(m.group(2))}, record_count=int(m.group(2)))
    if ext in (".mps",):
        rows = len(re.findall(r"(?m)^ [LEGN]  ", text))
        return _ok("mathmodel", "scientific", "mps_program",
                   {"format": "MPS", "row_count": rows}, record_count=rows)
    if ext in (".lp",):
        constraints = text.count("<=") + text.count(">=") + text.count("=")
        return _ok("mathmodel", "scientific", "lp_program",
                   {"format": "CPLEX LP", "constraint_ops": constraints})
    lines = [l for l in text.splitlines() if l.strip()]
    return _ok("mathmodel", "scientific", "math_program",
            ...
            if line.strip() and not line.startswith("%"):
                ...
                if len(parts) >= 3:
                    try:
                        ...
                        return _ok("sparsematrix", "scientific", "matrix_market",
                                   {"format": "Matrix Market", "rows": rows,
                                    "cols": cols, "nonzeros": nnz}, record_count=nnz)
                    except ValueError:
                        break
                break
    lines = [l for l in text.splitlines() if l.strip() and not l.startswith("%")]
    return _ok("sparsematrix", "scientific", "sparse_matrix",
...
def parse_ml_text(text: str, ext: str) -> Dict[str, Any]:
    if ext == ".arff":
        ...
        rows = 0
        if data_idx:
            rows = len([l for l in text[data_idx.end():].splitlines()
                        if l.strip() and not l.startswith("%")])
        cols = [{"name": a.strip("'\""), "dtype": "arff"} for a in attrs]
        return _ok("mltext", "text", "arff_dataset",
                   {"format": "Weka ARFF", "attribute_count": len(attrs),
                    "row_count": rows}, record_count=rows, columns=cols or None)
            ...
            for tok in l.split()[1:]:
                if ":" in tok:
                    try:
                        ...
                        pass
        return _ok("mltext", "text", "sparse_features",
                   {"format": "libsvm/VW", "row_count": len(rows),
                    "max_feature_index": max_feat or None}, record_count=len(rows))
    if ext in (".vocab", ".vec", ".merges", ".bpe"):
        lines = [l for l in text.splitlines() if l.strip()]
        first = lines[0].split() if lines else []
        is_w2v = len(first) == 2 and all(x.isdigit() for x in first)
        return _ok("mltext", "text", "vocabulary",
                   {"format": "vocab/embeddings", "token_count":
                    (int(first[0]) if is_w2v else len(lines)),
                    "declared_dim": int(first[1]) if is_w2v else None},
                   record_count=len(lines))
    if ext in (".prototxt", ".pbtxt"):
        layers = len(re.findall(r"(?m)^\s*layer\s*\{", text)) or \
            len(re.findall(r"(?m)^\s*node\s*\{", text))
        return _ok("mltext", "text", "proto_text",
                   {"format": "Protobuf text", "layer_count": layers},
                   record_count=layers)
    lines = [l for l in text.splitlines() if l.strip()]
    return _ok("mltext", "text", "ml_text",
               {"format": "ml text", "line_count": len(lines)}, record_count=len(lines))
...
def parse_spreadsheet_text(text: str, ext: str) -> Dict[str, Any]:
    if ext == ".dif":
        ...
        return _ok("spreadsheettext", "tabular", "dif_spreadsheet",
                   {"format": "DIF", "rows": int(m.group(1)) if m else None,
    ...
    if ext in (".sylk", ".slk"):
        ...
        return _ok("spreadsheettext", "tabular", "sylk_spreadsheet",
                   {"format": "SYLK", "cell_count": cells, "max_row": maxrow,
                    "max_col": maxcol}, record_count=cells)
    lines = [l for l in text.splitlines() if l.strip()]
    return _ok("spreadsheettext", "tabular", "text_spreadsheet",
...
def parse_geometry_text(text: str, ext: str) -> Dict[str, Any]:
    if ext in (".step", ".stp", ".p21"):
        ...
        return _ok("geometrytext", "scientific", "step_cad",
                   {"format": "STEP (ISO-10303)", "entity_count": entities,
    ...
    if ext in (".iges", ".igs"):
        ...
        return _ok("geometrytext", "scientific", "iges_cad",
                   {"format": "IGES", "directory_lines": d, "parameter_lines": p})
    if ext in (".dxf",):
        entities = text.count("\nENTITIES") and len(re.findall(r"(?m)^\s*0\s*$\n\s*(LINE|CIRCLE|ARC|POLYLINE|LWPOLYLINE|TEXT|INSERT)", text))
        return _ok("geometrytext", "scientific", "dxf_drawing",
                   {"format": "AutoCAD DXF", "entity_count": entities or None})
    if ext in (".wrl", ".vrml", ".x3d", ".x3dv"):
        nodes = len(re.findall(r"\b[A-Z]\w+\s*\{", text))
        return _ok("geometrytext", "scientific", "vrml_scene",
                   {"format": "VRML/X3D", "node_count": nodes}, record_count=nodes)
    if ext in (".ifc",):
        entities = len(re.findall(r"(?m)^#\d+\s*=", text))
        return _ok("geometrytext", "scientific", "ifc_bim",
                   {"format": "IFC", "entity_count": entities}, record_count=entities)
    lines = [l for l in text.splitlines() if l.strip()]
    return _ok("geometrytext", "scientific", "geometry_text",
...
def parse_postscript(text: str, ext: str) -> Dict[str, Any]:
    ...
    return _ok("postscript", "text", "postscript",
               {"format": "PostScript/EPS", "page_count": pages,
...
def parse_typeset(text: str, ext: str) -> Dict[str, Any]:
    if ext in (".bib",):
        ...
        return _ok("typeset", "text", "bibtex",
                   {"format": "BibTeX", "entry_count": len(entries),
                    "entry_types": dict(kinds) or None}, record_count=len(entries))
    if ext in (".dtd",):
        elements = len(re.findall(r"<!ELEMENT", text))
        attlists = len(re.findall(r"<!ATTLIST", text))
        entities = len(re.findall(r"<!ENTITY", text))
        return _ok("typeset", "text", "dtd_schema",
                   {"format": "DTD", "element_declarations": elements,
                    "attlist_declarations": attlists, "entity_declarations": entities},
    ...
    return _ok("typeset", "text", "tex_document",
               {"format": "TeX/LaTeX", "command_count": commands,
                "section_count": sections, "environment_count": envs,
                "package_count": len(packages), "packages": packages[:32] or None},
...
FAMILY_OF = _grp({
    "subtitle": (".srt", ".vtt", ".webvtt", ".sbv", ".ass", ".ssa", ".ttml",
                 ".dfxp", ".sami", ".smi", ".lrc", ".scc", ".mcc", ".cap",
                 ".aqt", ".jss", ".psb", ".rt", ".usf", ".sub"),
    "playlist": (".m3u", ".m3u8", ".pls", ".xspf", ".asx", ".wpl", ".b4s",
                 ".wax", ".cue", ".toc", ".ffp", ".sfv"),
    "rdf": (".ttl", ".n3", ".nt", ".nq", ".rdf", ".owl", ".trig", ".jsonld",
            ".trix", ".n-triples", ".nquads"),
    "sparql": (".rq", ".sparql", ".srx", ".srj"),
    "icalendar": (".ics", ".ical", ".ifb", ".vcs", ".vcard"),
    "email": (".eml", ".mbox", ".mbx", ".emlx"),
    "ldif": (".ldif",),
    "notebook": (".ipynb", ".rmd", ".qmd", ".jmd", ".zpln", ".myst"),
    "jsonfam": (".json5", ".jsonc", ".hjson", ".cson", ".geojsonl", ".har",
                ".ldjson"),
    "xmlfam": (".svg", ".xsl", ".xslt", ".xul", ".atom", ".rss", ".opml",
               ".plist", ".arxml", ".xbrl", ".ixbrl", ".fixml", ".ccda",
               ".musicxml", ".mml", ".mscx", ".fodt", ".fods", ".fodg",
               ".fodp", ".collada", ".sbml", ".cml", ".xaml",
               ".resx", ".wsdl", ".xsd", ".tcx", ".ubl",
               ".fb2", ".ncx", ".opf", ".acsm", ".xfdf", ".mathml", ".nzb",
               ".rels", ".vcproj", ".csproj", ".props", ".targets", ".nuspec",
               ".storyboard", ".xib", ".glif", ".ttx", ".qti", ".imscc"),
    "htmlfam": (".html", ".htm", ".shtml", ".shtm", ".xhtml", ".xht",
                ".hta", ".mht", ".mhtml"),
    "cssfam": (".css", ".scss", ".sass", ".less", ".styl", ".pcss"),
    "yamlfam": (".yaml", ".yml"),
    "edifam": (".edi", ".edifact", ".x12", ".edi850", ".edi810", ".edi837",
               ".edi835", ".edi856", ".hl7", ".hl7v2", ".ach", ".nacha",
               ".ncpdp", ".idoc"),
    "biotext": (".gb", ".gbk", ".genbank", ".embl", ".nwk", ".newick", ".tree",
                ".nex", ".nexus", ".phy", ".phylip", ".aln", ".sto",
                ".stockholm", ".mgf", ".wig", ".bedgraph", ".ped", ".fam",
                ".bim", ".map", ".gvcf", ".hmm", ".dnd"),
    "moltext": (".smiles", ".inchi", ".gro", ".poscar", ".contcar",
                ".vasp", ".psf", ".ent", ".pdbqt", ".mdl", ".prmtop",
                ".molden", ".fchk"),
    "chess": (".pgn", ".sgf", ".epd"),
    "sensortext": (".nmea", ".gps", ".ais", ".metar", ".taf", ".igc", ".gpsd"),
    "gerber": (".gbr", ".gerber", ".gbl", ".gtl", ".gts", ".gko",
               ".ger", ".xln", ".drl", ".exc", ".tap", ".pho", ".art"),
    "mathmodel": (".lp", ".mps", ".cnf", ".dimacs", ".tptp", ".smt2"),
    "sparsematrix": (".coord", ".hb"),
    "mltext": (".arff", ".libsvm", ".svmlight", ".vw", ".vocab", ".vec",
               ".merges", ".bpe", ".prototxt", ".liblinear"),
    "spreadsheettext": (".dif", ".sylk", ".slk"),
    "geometrytext": (".step", ".stp", ".p21", ".iges", ".igs", ".dxf", ".wrl",
                     ".vrml", ".x3dv", ".ifc", ".sat"),
    "postscript": (".ps", ".eps", ".epsf", ".epsi", ".ai"),
    "typeset": (".tex", ".latex", ".ltx", ".sty", ".cls", ".bib", ".bbl",
                ".bst", ".dtx", ".ins", ".dtd", ".texi", ".texinfo", ".rnw",
                ".ent", ".rng", ".sgm", ".sgml"),
})

_PARSERS = {
    "subtitle": parse_subtitle, "playlist": parse_playlist, "rdf": parse_rdf,
    "sparql": parse_sparql, "icalendar": parse_icalendar, "email": parse_email,
    "ldif": parse_ldif, "notebook": parse_notebook,
    "jsonfam": parse_json_tolerant, "xmlfam": parse_xml, "htmlfam": parse_html,
    "cssfam": parse_css, "yamlfam": parse_yaml, "edifam": parse_edi,
    "biotext": parse_bio_text, "moltext": parse_molecule_text,
    "chess": parse_chess, "sensortext": parse_sensor_text, "gerber": parse_gerber,
    "mathmodel": parse_math_model, "sparsematrix": parse_sparse_matrix,
    "mltext": parse_ml_text, "spreadsheettext": parse_spreadsheet_text,
    "geometrytext": parse_geometry_text, "postscript": parse_postscript,
...
def analyze(path: Any, ext: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return _ok(fam or "text", "text", "text_unreadable",
    ...
    if fam is None:
        ...
        return _ok("text", "text", "plain_text", m,
    ...
    try:
        ...
        return _ok(fam, "text", f"{fam}_partial", m,
                   record_count=m["line_count"], status="partial")
