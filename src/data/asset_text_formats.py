...
from .catalog_text_formats import _read_text, generic_text_metrics, _ok
...
# ---------------------------------------------------------------------------
_SHADER_EXTS = {
    ".cg", ".cginc", ".frag", ".geom", ".glsl", ".hlsl", ".metal", ".osl",
    ".rsl", ".shader", ".tesc", ".tese", ".ush", ".vert", ".wgsl",
}
_MLMODEL_EXTS = {
    ".state_dict", ".onnxtxt", ".param", ".relay", ".pojo", ".lm", ".nerf",
}
...
# ===========================================================================
_SHADER_LANG = {
    ".vert": "glsl", ".frag": "glsl", ".geom": "glsl", ".tesc": "glsl",
    ".tese": "glsl", ".glsl": "glsl",
    ".hlsl": "hlsl", ".ush": "hlsl",
    ".metal": "metal",
    ".cg": "cg", ".cginc": "cg",
    ".osl": "osl", ".rsl": "renderman",
    ".shader": "shaderlab", ".wgsl": "wgsl",
}
...
def _parse_shader(text: str, ext: str) -> Dict[str, Any]:
    ...
    if lang == "wgsl":
        for grp, binding, name, ty in _WGSL_BIND.findall(src):
            bindings += 1
            ty = ty.strip()
            columns.append({"name": name, "dtype": ty,
                            "qualifier": f"group{grp}/binding{binding}"})
            if "sampler" in ty:
                samplers += 1
            elif "texture" in ty:
                textures += 1
        ...
        for st, tag in (("vertex", "vertex"), ("fragment", "fragment"),
                        ("geometry", "geometry"), ("compute", "kernel")):
        ...
        for kw in ("vertex", "fragment", "kernel"):
            for fm in re.finditer(r"(?m)^\s*%s\s+[\w:<>,\s\*&]+?\s+([A-Za-z_]\w*)\s*\(" % kw, src):
                entrypoints.append(fm.group(1))
        functions = [f[1] for f in _GLSL_FUNC.findall(src)]
    elif lang in ("osl", "renderman"):
        for mm in re.finditer(r"(?m)^\s*(shader|surface|displacement|light|volume)\s+([A-Za-z_]\w*)", src):
            entrypoints.append(mm.group(2))
        for mm in re.finditer(r"\b(output\s+)?(color|point|vector|normal|float|int|string|matrix)\s+([A-Za-z_]\w*)\s*[=,)\[]", src):
            columns.append({"name": mm.group(3), "dtype": mm.group(2),
    ...
    return _ok("shader", "code", "shader_source", props,
               record_count=len(columns), columns=columns or None)
...
def _parse_ncnn_param(text: str) -> Dict[str, Any]:
    ...
    for ln in lines[idx:]:
        ...
        layers.append({"layer_name": lname, "layer_type": ltype,
                       "ordinal": len(layers)})
    props = {"format": "NCNN param", "magic": magic,
             "declared_layer_count": layer_count, "declared_blob_count": blob_count,
             "parsed_layer_count": len(layers),
             "distinct_layer_types": len(type_hist),
             "layer_type_histogram": type_hist or None}
    return {"props": props, "record_count": len(layers), "model_layers": layers,
            "subcategory": "ncnn_param", "status": "ok"}
...
def _parse_arpa(text: str) -> Dict[str, Any]:
    ...
    return {"props": props, "record_count": sum(counts.values()) if counts else 0,
            "columns": cols or None, "subcategory": "arpa_lm", "status": "ok"}
...
def _parse_state_dict_text(text: str) -> Dict[str, Any]:
    ...
    for key, shape_s in _STATE_DICT_LINE.findall(text):
        ...
        if not dims:
            continue
        t = {"name": key, "dtype": None, "shape": dims,
             "num_bytes": None, "extra": {}}
        tensors.append(t)
    ...
    for i, (lname, ts) in enumerate(layer_tensors.items()):
        ...
        for t in ts:
            ...
            for d in t["shape"]:
                n *= d
            total += n
        layers.append({"layer_name": lname, "layer_type": "module",
                       "ordinal": i, "tensor_count": len(ts),
                       "total_parameters": total})
    props = {"format": "torch state_dict (text)",
             "tensor_count": len(tensors), "layer_count": len(layers),
             "total_parameters": sum(l["total_parameters"] for l in layers)}
    return {"props": props, "record_count": len(tensors), "tensors": tensors,
            "model_layers": layers, "subcategory": "state_dict", "status": "ok"}
...
def _parse_onnxtxt(text: str) -> Dict[str, Any]:
    ...
    props = {"format": "ONNX text",
             "graph_name": gname.group(1) if gname else None,
             "node_count": node_count, "input_count": inputs or None,
             "output_count": outputs or None, "initializer_count": inits or None,
             "distinct_ops": len(op_hist), "op_histogram": op_hist or None}
    cols = [{"name": op, "dtype": "op"} for op in sorted(op_hist)]
    return {"props": props, "record_count": node_count, "columns": cols or None,
            "subcategory": "onnx_text", "status": "ok"}
...
def _parse_relay(text: str) -> Dict[str, Any]:
    ...
    props = {"format": "TVM Relay IR",
             "relay_version": ver.group(1) if ver else None,
             "function_count": len(funcs), "var_count": var_count,
             "op_call_count": sum(ops.values()), "distinct_ops": len(ops),
             "op_histogram": ops or None}
    cols = [{"name": f, "dtype": "relay_fn"} for f in funcs]
    return {"props": props, "record_count": len(funcs), "columns": cols or None,
            "subcategory": "tvm_relay", "status": "ok"}


def _parse_pojo(text: str) -> Dict[str, Any]:
    """H2O POJO: generated Java GenModel with per-tree static methods/arrays."""
    classes = re.findall(r"(?m)^\s*(?:public\s+|final\s+|abstract\s+)*class\s+(\w+)", text)
    main_class = classes[0] if classes else None
    trees = len(re.findall(r"\bclass\s+\w*_Tree_\w*", text)) + \
        len(re.findall(r"\bstatic\s+final\s+void\s+\w*Tree\w*\s*\(", text))
    methods = len(re.findall(r"(?m)^\s*(?:public|private|protected|static|final|\s)+"
                             r"[\w\[\]<>]+\s+(\w+)\s*\([^)]*\)\s*\{", text))
    ...
    props = {"format": "H2O POJO (Java)", "model_class": main_class,
             "class_count": len(classes), "method_count": methods,
             "static_array_count": arrays, "tree_count": trees or None,
             "h2o_genmodel": is_h2o}
    return {"props": props, "record_count": len(classes),
            "subcategory": "h2o_pojo", "status": "ok"}
...
def _parse_nerf(text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"props": m, "record_count": m["line_count"],
                "subcategory": "nerf_config", "status": "partial"}
    props: Dict[str, Any] = {"format": "NeRF/instant-ngp config"}
    frames = None
    if isinstance(obj, dict):
        for k in ("camera_angle_x", "fl_x", "fl_y", "cx", "cy", "w", "h",
                  "aabb_scale", "n_levels", "resolution"):
            if k in obj:
                props[k] = obj[k]
        fr = obj.get("frames")
        ...
        frames = len(obj)
        props["frame_count"] = frames
    return {"props": props, "record_count": frames if frames is not None else 0,
            "subcategory": "nerf_config", "status": "ok"}
...
def _parse_mlmodel(text: str, ext: str) -> Dict[str, Any]:
    ...
    if not res:  # unrecognised content -> honest generic metrics
        ...
        res = {"props": m, "record_count": m["line_count"],
               "subcategory": "ml_model_text", "status": "partial"}
    out = _ok("ml_model", "tensor", res.get("subcategory", "ml_model_text"),
              res["props"], record_count=res.get("record_count"),
              columns=res.get("columns"), status=res.get("status", "ok"))
    if res.get("tensors"):
...
def _parse_vector(text: str, ext: str) -> Dict[str, Any]:
    if ext == ".excalidraw":
        try:
            ...
            return _ok("vector_graphics", "image", "vector_scene_partial", m,
        ...
        return _ok("vector_graphics", "image", "vector_scene", props,
                   record_count=len(elements), columns=cols or None)
    ...
    beziers = len(re.findall(r"(?m)^\s*b\s*\(", text)) + \
        len(re.findall(r"(?m)^\s*bp\s*\(", text))
    ...
    props = {"format": header or "Sketch/sK1 vector script",
             "layer_count": layers, "bezier_count": beziers,
             "rect_count": rects, "ellipse_count": ellipses,
             "group_count": groups,
             "object_count": layers + beziers + rects + ellipses}
    return _ok("vector_graphics", "image", "vector_drawing", props,
...
def _parse_worldfile(text: str, ext: str) -> Dict[str, Any]:
    ...
    if len(vals) < 6:
        ...
        return _ok("geospatial", "geospatial", "world_file_partial", m,
                   record_count=len(vals), status="partial")
    a, d, b, e, c, f = vals[:6]
    props = {"format": "ESRI world file",
             "pixel_size_x": a, "rotation_y": d, "rotation_x": b,
             "pixel_size_y": e, "origin_x": c, "origin_y": f,
             "has_rotation": bool(d or b),
             "coefficient_count": 6}
    cols = [{"name": n, "dtype": "float"} for n in
            ("A_pixel_size_x", "D_rotation_y", "B_rotation_x",
             "E_pixel_size_y", "C_origin_x", "F_origin_y")]
    return _ok("geospatial", "geospatial", "world_file", props,
               record_count=6, columns=cols)
...
def _parse_ringtone(text: str, ext: str) -> Dict[str, Any]:
    ...
    if len(parts) < 3:
        ...
        return _ok("ringtone", "audio", "rtttl_partial", m,
                   record_count=m["line_count"], status="partial")
    name = parts[0].strip()
    ...
    props = {"format": "RTTTL", "tune_name": name or None,
...
def _parse_vtk(text: str, ext: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return _ok("model_3d", "three_d", "vtk_partial", m,
                   record_count=m["line_count"], status="partial")
    ...
    grid = None
    for child in root:
        if _local(child.tag) in ("RectilinearGrid", "StructuredGrid",
                                  "ImageData", "UnstructuredGrid", "PolyData"):
            grid = child
            break
    ...
    props = {"format": f"VTK XML {vtk_type or _local(grid.tag) if grid is not None else 'grid'}",
             "vtk_type": vtk_type, "version": version, "byte_order": byte_order,
             "whole_extent": whole_extent, "dimensions": dims,
             "point_count": npoints,
             "data_array_count": len(columns),
             "point_data_arrays": point_arrays or None,
             "cell_data_arrays": cell_arrays or None}
    out = _ok("model_3d", "three_d", "vtk_grid", props,
...
def analyze(path: Any, ext: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return _ok(fam or "asset", "text", "asset_unreadable",
