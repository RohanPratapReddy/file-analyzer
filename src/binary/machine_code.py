...
class MachineCodeAnalyzer:
    ...
    ROUTED_EXTS = frozenset({
        ...
        # PE / COFF (Windows)
        ".exe", ".dll", ".sys", ".efi", ".ocx", ".cpl", ".scr", ".drv", ".mui",
        ".winmd",
    ...
    def __init__(self, engine: Any = None, files_rows: Optional[List[Dict[str, Any]]] = None):
        """
        Args:
    ...
    def process(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for row in self.files_rows:
            ...
            bid = self._bid
            try:
                self._analyze_one(bid, row, index, sections, symbols, imports, properties)
            except Exception as err:  # noqa: BLE001 - one bad file must not sink the batch
                index.append(self._min_index_row(bid, row, error=f"{type(err).__name__}: {err}"))
    ...
    # ------------------------------------------------------------------
    def _analyze_one(self, bid, row, index, sections, symbols, imports, properties) -> None:
        ...
        idx["export_count"] = sum(1 for s in symbols if s["binary_id"] == bid and s.get("is_export"))
        index.append(idx)
    ...
    def _dispatch(self, head: bytes, ext: str):
        ...
        if head[:4] in (b"\xca\xfe\xba\xbe", b"\xca\xfe\xba\xbf") and ext in (".class",):
    ...
    # ==================================================================
    _ELF_MACHINE = {
        0: "None", 2: "SPARC", 3: "x86", 8: "MIPS", 20: "PowerPC",
        21: "PowerPC64", 40: "ARM", 42: "SuperH", 43: "SPARCv9",
        50: "IA-64", 62: "x86-64", 183: "AArch64", 243: "RISC-V",
        258: "LoongArch",
    }
    _ELF_OSABI = {
        0: "System V", 1: "HP-UX", 2: "NetBSD", 3: "Linux", 6: "Solaris",
        9: "FreeBSD", 12: "OpenBSD", 255: "Standalone",
    }
    _ELF_TYPE = {0: "NONE", 1: "REL (object)", 2: "EXEC", 3: "DYN (shared/PIE)", 4: "CORE"}
    _ELF_SHT = {
    ...
    def _parse_elf(self, bid, path, idx, sections, symbols, imports, properties):
        ...
        idx["is_pic"] = (e_type == 3)
        self._prop(properties, bid, "elf", "type", self._ELF_TYPE.get(e_type, str(e_type)))
        self._prop(properties, bid, "elf", "osabi", self._ELF_OSABI.get(ei_osabi, str(ei_osabi)))
        ...
        if e_shoff and e_shnum and e_shnum < _MAX_SECTIONS:
                ...
                if bits == 64:
                    (sh_name, sh_type, sh_flags, sh_addr, sh_offset, sh_size,
                     sh_link, sh_info, sh_addralign, sh_entsize) = struct.unpack(
                        endian + "IIQQQQIIQQ", raw[:64])
                else:
                    (sh_name, sh_type, sh_flags, sh_addr, sh_offset, sh_size,
                     sh_link, sh_info, sh_addralign, sh_entsize) = struct.unpack(
                        endian + "IIIIIIIIII", raw[:40])
                sh_entries.append((sh_name, sh_type, sh_flags, sh_addr, sh_offset, sh_size))
            ...
            for ordv, (sh_name, sh_type, sh_flags, sh_addr, sh_offset, sh_size) in enumerate(sh_entries):
                ...
                sections.append({
                    ...
                    "entropy": self._section_entropy(data, sh_offset, sh_size, sh_type),
                })
    ...
    def _elf_shflags(self, f):
        out = []
        if f & 0x1: out.append("WRITE")
        if f & 0x2: out.append("ALLOC")
        if f & 0x4: out.append("EXEC")
        if f & 0x20: out.append("STRINGS")
        return ",".join(out)
    ...
    _PE_MACHINE = {
        ...
        0x1c4: "ARMv7 (Thumb)", 0x200: "IA-64", 0x5032: "RISC-V32",
        0x5064: "RISC-V64", 0xebc: "EFI byte code",
    }
    ...
    def _parse_pe(self, bid, path, idx, sections, symbols, imports, properties):
        ...
        idx["format_family"] = "executable"
        coff = e_lfanew + 4
        (machine, num_sections, timestamp, sym_ptr, num_syms,
         opt_size, characteristics) = struct.unpack("<HHIIIHH", data[coff:coff + 20])
        ...
        opt = coff + 20
        magic = struct.unpack("<H", data[opt:opt + 2])[0] if opt + 2 <= len(data) else 0
        ...
        for i in range(min(num_sections, _MAX_SECTIONS)):
            ...
            (vsize, vaddr, rawsize, rawptr, relptr, lnptr,
             nrel, nln, chars) = struct.unpack("<IIIIIIHHI", raw[8:40])
            self._sec_id += 1
            sections.append({
                "section_id": self._sec_id,
                "binary_id": bid,
                "ordinal": i,
                "name": name,
                "sec_type": "code" if chars & 0x20 else ("data" if chars & 0xC0 else "other"),
    ...
    def _pe_secflags(self, c):
        out = []
        if c & 0x20: out.append("CODE")
        if c & 0x40: out.append("IDATA")
        if c & 0x80: out.append("UDATA")
        if c & 0x20000000: out.append("EXEC")
        if c & 0x40000000: out.append("READ")
        if c & 0x80000000: out.append("WRITE")
        return ",".join(out)
    ...
    def _pe_exports(self, bid, data, opt, pe32plus, sec_off, num_sections, symbols):
        ...
        try:
            ...
            for i in range(min(num_names, _MAX_SYMBOLS)):
                name_ptr = struct.unpack("<I", data[names_off + i * 4:names_off + i * 4 + 4])[0]
                ...
                self._sym_id += 1
                symbols.append({
                    "symbol_id": self._sym_id, "binary_id": bid, "name": name,
                    "sym_kind": "export", "binding": None, "address": None,
                    "size": None, "section": None, "is_import": False,
                    "is_export": True, "library": None,
                })
        except (struct.error, IndexError):
            return
    ...
    # Mach-O
    # ==================================================================
    _MACHO_CPU = {7: "x86", 0x01000007: "x86-64", 12: "ARM",
                  0x0100000c: "ARM64", 18: "PowerPC", 0x01000012: "PowerPC64"}
    _MACHO_FILETYPE = {1: "OBJECT", 2: "EXECUTE", 6: "DYLIB", 7: "DYLINKER",
                       8: "BUNDLE", 9: "DYLIB_STUB", 0xa: "DSYM"}
    ...
    def _parse_macho(self, bid, path, idx, sections, symbols, imports, properties):
        ...
        idx["architecture"] = self._MACHO_CPU.get(cputype & 0xffffffff, f"cpu-{cputype}")
        idx["is_pic"] = bool(flags & 0x200000)   # MH_PIE
        idx["is_dynamic"] = (filetype in (6, 8))
        self._prop(properties, bid, "macho", "filetype",
        ...
        for _ in range(min(ncmds, _MAX_SECTIONS)):
            ...
            if cmd in (0x1, 0x19):
                ...
                if is_seg64:
                    vmaddr, vmsize, fileoff, filesize = struct.unpack(endian + "QQQQ", seg[24:56])
                    nsects = struct.unpack(endian + "I", seg[64:68])[0]
                else:
                    vmaddr, vmsize, fileoff, filesize = struct.unpack(endian + "IIII", seg[24:40])
                    nsects = struct.unpack(endian + "I", seg[48:52])[0]
                self._sec_id += 1
                ordv += 1
                sections.append({
                    "section_id": self._sec_id, "binary_id": bid, "ordinal": ordv,
                    "name": segname, "sec_type": "segment",
                    "virtual_address": vmaddr, "file_offset": fileoff,
                    "size": filesize, "flags": "",
                ...
                if lib:
                    self._imp_id += 1
                    imports.append({
                        "import_id": self._imp_id, "binary_id": bid,
                        "library": lib, "symbol": None, "kind": "load-dylib",
                    })
    ...
    def _macho_symtab(self, bid, data, endian, is64, cmd_off, symbols):
        ...
        for i in range(min(nsyms, _MAX_SYMBOLS)):
            ...
            if len(raw) < entsize:
                break
            if is64:
                n_strx, n_type, n_sect, n_desc, n_value = struct.unpack(endian + "IBBHQ", raw)
            else:
                n_strx, n_type, n_sect, n_desc, n_value = struct.unpack(endian + "IBBHI", raw)
            ...
            symbols.append({
                "symbol_id": self._sym_id, "binary_id": bid, "name": name,
                "sym_kind": "ext" if is_ext else "local", "binding": None,
                "address": n_value, "size": None, "section": str(n_sect),
    ...
    def _parse_macho_fat(self, bid, path, idx, sections, symbols, imports, properties):
        ...
        for i in range(min(nfat, 64)):
            ...
            self._prop(properties, bid, "macho",
                       f"slice{i}_arch",
    ...
    def _parse_class(self, bid, path, idx, sections, symbols, imports, properties):
        ...
        self._prop(properties, bid, "java", "jdk", self._JAVA_VERSION.get(major, "unknown"))
        ...
        try:
            while i < cp_count and off < len(data):
                ...
                if tag == 1:  # Utf8
                    ...
                    off += ln
                elif tag in (7, 8, 16, 19, 20):  # Class/String/MethodType/Module/Package (u2 index)
                    if tag == 7:
        ...
        for ref in class_refs[:_MAX_IMPORTS]:
            ...
            if cname and cname not in seen:
                ...
                imports.append({
                    "import_id": self._imp_id, "binary_id": bid,
                    "library": cname.replace("/", "."), "symbol": None,
                    "kind": "class-ref",
                })
    ...
    def _parse_wasm(self, bid, path, idx, sections, symbols, imports, properties):
        ...
        while off < len(data):
            ...
            sections.append({
                ...
                "sec_type": "wasm-section", "virtual_address": None,
                "file_offset": body_start, "size": size, "flags": "",
    ...
    def _wasm_imports(self, bid, data, start, size, imports):
        ...
        try:
            ...
            for _ in range(min(count, _MAX_IMPORTS)):
                ...
                imports.append({
                    "import_id": self._imp_id, "binary_id": bid,
                    "library": module, "symbol": field,
                    "kind": {0: "func", 1: "table", 2: "memory", 3: "global"}.get(kind, "import"),
                })
        except (struct.error, IndexError):
            return
    ...
    def _parse_ar(self, bid, path, idx, sections, symbols, imports, properties):
        ...
        while off + 60 <= len(data):
            ...
            sections.append({
                "section_id": self._sec_id, "binary_id": bid, "ordinal": ordv,
                "name": name, "sec_type": "ar-member", "virtual_address": None,
                "file_offset": off + 60, "size": size, "flags": "", "entropy": None,
            })
            off += 60 + size + (size & 1)
    ...
    # ==================================================================
    def _parse_llvm_bitcode(self, bid, path, idx, sections, symbols, imports, properties):
        idx["binary_format"] = "LLVM bitcode"
        idx["format_family"] = "bytecode"
        idx["architecture"] = "LLVM IR"
        head = path.read_bytes()[:4]
        self._prop(properties, bid, "llvm", "wrapper",
                   str(head == b"\xde\xc0\x17\x0b"))
    ...
    def _parse_uf2(self, bid, path, idx, sections, symbols, imports, properties):
        ...
        if len(data) >= 32:
            flags, target_addr, payload_size, block_no, num_blocks, family = struct.unpack(
                "<IIIIII", data[8:32])
    ...
    def _parse_ole(self, bid, path, idx, sections, symbols, imports, properties):
        ...
        self._prop(properties, bid, "ole", "note", "structure only; no stream payload read")
    ...
    def _prop(self, properties, bid, group, name, value):
        self._prop_id += 1
        properties.append({
            "property_id": self._prop_id, "binary_id": bid,
            "prop_group": group, "prop_name": name, "prop_value": str(value),
        })
    ...
    def _min_index_row(self, bid, row, error=None):
        return {
            "binary_id": bid, "file_id": row.get("file_id"),
            "file_name": Path(row.get("file_location", "")).name,
            "binary_format": None, "format_family": None,
            "category": None, "subcategory": None,
            "architecture": None, "bitness": None, "endianness": None,
            "entry_point": None, "is_stripped": None, "is_dynamic": None,
            "is_pic": None, "section_count": 0, "symbol_count": 0,
            "import_count": 0, "export_count": 0, "sha256": None, "size": None,
            "entropy": None, "detected_via": None, "notes": None, "error": error,
        }
