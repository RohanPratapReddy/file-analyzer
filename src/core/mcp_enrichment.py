...
* **Soft agent enrichment (added only when reachable).** Using the same
  :mod:`src.document.agent_mcp` transport the dynamic layer uses (with
...
bare interpreter); :mod:`src.document.agent_mcp` is imported lazily inside the
...
class McpEnrichmentDatabaseGenerator:
    ...
    Mirrors :class:`src.document.document_db.DocumentParserDatabaseGenerator`:
