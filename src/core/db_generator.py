...
EXECUTE FUNCTION {self.schema_name}.update_timestamp_column();""")
    ...
    def export_to_sqlite_db_concurrent(self, db_path: str = "repository.db",
        ...
        # 2. Inject per-table INSERT blocks concurrently.
        blocks = self._split_inserts_into_blocks()

        def _inject(block: Tuple[str, str]) -> Tuple[str, Optional[str]]:
            label, sql_text = block
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as ex:
            for label, err in ex.map(_inject, blocks):
                if err is not None:
