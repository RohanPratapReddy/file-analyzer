...
# ----------------------------------------------------------------------------
# Canonical extension catalog (src/tables/file_extensions.json)
# ----------------------------------------------------------------------------
...
def _load_extension_catalog(path: Union[str, Path]) -> Optional[Dict[str, int]]:
    """
    Load ``src/tables/file_extensions.json`` into a ``{extension_name -> extension_id}``
...
# ``src/tables/`` (built verbatim from the IANA tz database, data.iana.org):
...
# ----------------------------------------------------------------------------
# IANA timezone tables (src/tables/iana_{local,global}_timezones.json)
# ----------------------------------------------------------------------------
...
def _tables_dir() -> Path:
    """The ``src/tables/`` directory that ships the reference tables."""
...
def resolve_timezone64(
    ...
    ``src/tables/iana_*_timezones.json``, or ``None`` when no row carries that slot (an
...
class RepositoryAnalyzer:
    ...
    def __init__(
        ...
        # ``src/tables/file_extensions.json`` inside the package
        # (this module lives at src/core/, so parents[1] is the src/ package root).
