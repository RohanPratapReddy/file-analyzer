"""
``python -m src`` entrypoint -- run the repository monitoring layer as a process.
...
Examples::
    ...
    python -m src monitor /repo --once          # one scan cycle, then exit
...
The subcommand is optional -- ``python -m src /repo`` is treated as
``python -m src monitor /repo``.
"""
...
def _split_names(value: Optional[str]) -> Optional[List[str]]:
    ...
    as "no filter"). Mirrors ``src.main._split_names``.
    """
    if not value:
        return None
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    require_virtualized(context="python -m src")
