"""
Dispatch layer for the ``misc`` analysis plane.
...
``profile`` dict in the exact canonical shape the MiscAnalyzer flattens
...
the router derives the ``misc`` route set from it after subtracting every
...
_REGISTRY: Dict[str, _p.MiscTypeParser] = _p.build_registry()


def known_exts() -> frozenset:
    """Every extension the misc plane owns (lower-case, dot-prefixed)."""
...
def analyze(path: str, ext: str) -> Dict[str, Any]:
    """Parse one misc file and return its normalized profile dict."""
    e = ext.lower()
