"""
TextualAnalyzer -- one text-record *file* decomposed into normalized,
relational document/section/record/field tables.

For every extension of the seven *text-record* content kinds still residual in
``DUMP/tabgen/docs/residual.json`` (data_text, text, log, documentation,
template, scientific_data, subtitle) this engine runs a real, pure-stdlib,
structure-aware parser (see :mod:`.textual_formats`) that decomposes the file
into the canonical shape:

    document -> sections -> records -> fields

and flattens it into a fully-normalized set of tables:

  * one **section** row per top-level grouping (a caption track, a log event
    stream, a man-page ``.SH`` block, a template's directive list, a
    bibliographic entry, an EDI segment group, ...),
  * one **record** row per entry inside a section (a caption cue, a log line,
    a checksum, a G-code command, a JSON object, a table row, ...),
  * one **field** row per typed name/value pair on a record.

Honesty contract (identical to :class:`..config.config_analyzer.ConfigAnalyzer`):
content is sniffed first, an inherently-binary payload degrades to an honest
forensic byte profile with no fabricated records, credential material (JWT
signatures) is redacted and never decoded, the raw payload is never stored, and
a parse that only partially succeeds is reported ``partial`` -- never stubbed.
...
``text_file_index`` -- exactly like the schema/database/data/config layers -- so
the plane plugs straight into ``RepositoryDatabaseGenerator``.
  ...
  text_files_table      text_file_id, file_name, extension, content_kind,
                        ...
                        section_count, record_count, field_count,
  ...
  text_fields_table     text_field_id, text_record_id, text_file_id,
                        text_section_id, field_name, field_key, field_type,
...
class TextualAnalyzer:
    """Analyze text-record files into normalized ``text_*`` tables."""
    ...
    def __init__(
        ...
        dump_file_path: str = "text_analysis.json",
        ...
        self.text_files_table: List[Dict[str, Any]] = []
        self.text_sections_table: List[Dict[str, Any]] = []
        self.text_records_table: List[Dict[str, Any]] = []
        self.text_fields_table: List[Dict[str, Any]] = []
        self.text_properties_table: List[Dict[str, Any]] = []
        self.text_file_index: List[Dict[str, Any]] = []

        self._ids = {k: 0 for k in (
            "file", "section", "record", "field", "property", "tfi",
        )}
    ...
    def _known_exts(cls) -> frozenset:
        return textual_formats.known_exts()

    @classmethod
    def routing_suffixes(cls) -> Tuple[str, ...]:
        return textual_formats.routing_suffixes()
    ...
    def analyze(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for local_fid, path in enumerate(self.file_paths, start=1):
            ...
            if ext not in exts:
                continue
            try:
                profile = textual_formats.analyze(path, ext)
            except Exception as err:  # never let one bad file abort the batch
                print(f"Warning: TextualAnalyzer failed on {path.name}: {err}")
                continue
            try:
                self._emit_file(path, ext, profile, local_fid)
            except Exception as err:
                print(f"Warning: TextualAnalyzer emit failed on {path.name}: {err}")
                continue
    ...
    def get_tables(self) -> Dict[str, List[Dict[str, Any]]]:
        return {
            "text_files_table": self.text_files_table,
            "text_sections_table": self.text_sections_table,
            "text_records_table": self.text_records_table,
            "text_fields_table": self.text_fields_table,
            "text_properties_table": self.text_properties_table,
            "text_file_index": self.text_file_index,
        }
        ...
        for sec in (profile.get("sections") or []):
            ...
            for rindex, rec in enumerate(sec.get("records") or []):
                ...
                fld_count += self._emit_record(
                    text_file_id, section_id, rindex, rec, local_fid)
            self.text_sections_table[-1]["record_count"] = local_recs
    ...
    def _emit_section(self, tfid: int, sec: Dict[str, Any], local_fid: int) -> int:
        section_id = self._next("section")
        self.text_sections_table.append({
    ...
    def _emit_record(self, tfid: int, section_id: int, rindex: int,
                     rec: Dict[str, Any], local_fid: int) -> int:
        record_id = self._next("record")
        fields = rec.get("fields") or []
        self.text_records_table.append({
            "text_record_id": record_id,
            "text_file_id": tfid,
            "text_section_id": section_id,
            "record_index": rindex,
        ...
        n = 0
        for fld in fields:
            if self._emit_field(tfid, section_id, record_id, fld, local_fid):
                n += 1
        return n

    def _emit_field(self, tfid: int, section_id: int, record_id: int,
                    fld: Dict[str, Any], local_fid: int) -> bool:
        if not isinstance(fld, dict):
            return False
        self.text_fields_table.append({
            "text_field_id": self._next("field"),
            "text_record_id": record_id,
            "text_file_id": tfid,
            "text_section_id": section_id,
    ...
    def _emit_property(self, tfid: int, entry: Any, local_fid: int) -> bool:
        ...
        if pv is None:
            return False
        self.text_properties_table.append({
    ...
    # ------------------------------------------------------------------
    # coercion helpers (mirror ConfigAnalyzer)
    # ------------------------------------------------------------------
    @staticmethod
    ...
    # ==================================================================
    # Repository linkage (mirrors ConfigAnalyzer.link_repository)
    # ==================================================================
    def link_repository(
        self,
        ...
        entity_tables = [
            (self.KIND_FILE, self.text_files_table, "text_file_id"),
            (self.KIND_SECTION, self.text_sections_table, "text_section_id"),
            (self.KIND_RECORD, self.text_records_table, "text_record_id"),
            (self.KIND_FIELD, self.text_fields_table, "text_field_id"),
            (self.KIND_PROPERTY, self.text_properties_table, "property_id"),
        ]
        self.text_file_index.clear()
        for kind, rows, id_key in entity_tables:
            for row in rows:
                repo_id = local_to_repo.get(row.get("file_id"))
                row["file_id"] = repo_id
                if repo_id is not None:
                    self.text_file_index.append({
                        "tfi_id": self._next("tfi"), "file_id": repo_id,
                        "entity_kind": kind, "entity_id": row[id_key],
                    })
        return self.text_file_index
    ...
    def _export(self, result: Dict[str, List[Dict[str, Any]]]) -> None:
        try:
            ...
            print(f"Warning: TextualAnalyzer export failed: {err}")
