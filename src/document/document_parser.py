...
(``src/tables/file_extensions.json``) -- word-processor documents, e-books,
...
:func:`src.router.routing.resolve_analyzer` and ``src/router/worker.py``) so every
...
# lifted verbatim from src/tables/file_extensions.json. This is the plane's
...
# --- code-source families (from src/tables/file_extensions.json extension_type) -
...
class DocumentParser:
    ...
    :class:`src.document.document_db.DocumentParserDatabaseGenerator`, which
    ...
    def dynamic_analyze(
        ...
        *,
        ...
        Builds one :class:`~src.document.dynamic_engine.DocInput` per analyzed
        ...
        :class:`~src.document.agent_mcp.AgentRegistry`; when a stage has no
        reachable provider it falls back to the deterministic
        :class:`~src.document.dynamic_engine.HeuristicRunner`, so this method
    ...
    def evaluate(
        ...
        *,
        ...
        :class:`~src.document.evaluation_engine.EvalCase` (text/OCR, table,
        ...
        from :mod:`~src.document.evaluation_metrics`; the C3 RAG family (the
