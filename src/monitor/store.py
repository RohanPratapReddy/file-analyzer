...
The FIFO diff database (:mod:`src.monitor.diff_db`) is deliberately SQLite-only
and local -- it is a rolling 16-deep ring. The two *durable* dumps, however, must
be storable **on any remote server**:

* the continuous **change log** (:mod:`src.monitor.change_log`) -- every change,
  forever, so nothing is lost when the FIFO ring recycles a slot;
* the **session summary** (:mod:`src.monitor.session_log`) -- one row per agent
...
``src.monitor`` package) still works on a bare interpreter with nothing installed
