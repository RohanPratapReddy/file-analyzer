...
* the real fast path is a **Go worker pool** (``src/monitor/go/plane.go``): a
  fixed set of goroutines drains a channel of batches and spawns one
  ``python -m src.monitor.worker`` child process per batch, giving genuine
...
class UpdateWorkerPool:
    def __init__(
        ...
        self.go_dir = self.readers_root / "src" / "monitor" / "go"
        self.log: List[str] = []
