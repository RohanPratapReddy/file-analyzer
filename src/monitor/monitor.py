...
* :meth:`run_forever` runs the loop in the foreground with SIGINT/SIGTERM
  handlers and a PID file -- used by ``python -m src`` so the layer persists as
  its own process (see ``src/__main__.py``).
...
class RepositoryMonitor:
    def __init__(
        ...
        # ``readers_root`` is the directory containing the ``src`` package.
        self.readers_root = (
    ...
    def run_forever(self, pid_file: Optional[os.PathLike] = None) -> int:
        ...
        Used by ``python -m src`` so the monitoring layer persists as its own
