...
Routing is done by :func:`src.router.routing.resolve_analyzer`, the same function
...
# src/router/worker.py). archive / binary are intentionally absent -- they have
...
def _engines(readers_root: Optional[str] = None) -> Dict[str, Any]:
    ...
    from src import (  # noqa: E402  (heavy fleet import, deferred to first use)
        ConfigAnalyzer,
        DataAnalyzer,
        DatabaseAnalyzer,
        DocumentAnalyzer,
...
class IncrementalUpdateEngine:
    ...
    (:class:`src.core.mcp_enrichment.McpEnrichmentEngine`) -- the very same
    ...
    def resolve_class(self, path: str) -> Optional[str]:
        from src.router.routing import resolve_analyzer
    ...
    def _run_agent_tier(
        ...
        try:
            from src.core.mcp_enrichment import McpEnrichmentEngine
