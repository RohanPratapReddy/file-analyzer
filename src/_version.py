...
* ``pyproject.toml`` can read it statically via setuptools' ``attr:`` directive
  (``[tool.setuptools.dynamic] version = {attr = "src._version.__version__"}``),
...
* the MCP server (``mcp_server.py``) can recover the version by parsing this file
  directly, without triggering ``src/__init__.py`` (which would import the whole
  analyzer fleet and slow the agent handshake);
* the SDK exposes it as ``src.__version__`` and the CLIs expose it via
  ``--version``.
