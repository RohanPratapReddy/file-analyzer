...
class InnoSetupAnalyzer(ShellScriptBase):
    ...
    _PROC = re.compile(r"(?mi)^[ \t]*(procedure|function)[ \t]+([A-Za-z_]\w*)[ \t]*(?:\(([^)]*)\))?")
    ...
    def _extract_entities(self, file_id, text, path):
        ...
        self._record_module_meta(file_id, sections=sorted(seen_cls) or None,
                                 code_functions=len(seen_fn), defines=len(seen_var))
...
class NsisAnalyzer(ShellScriptBase):
    ...
    _SECTION = re.compile(r'(?mi)^[ \t]*Section(?:Group)?[ \t]+(?:/o[ \t]+)?'
                          r'(?:"([^"]*)"|(\S+))')
    _DEFINE = re.compile(r"(?mi)^[ \t]*!define[ \t]+(\w+)[ \t]*(.*)")
    _VAR = re.compile(r"(?mi)^[ \t]*Var[ \t]+(?:/GLOBAL[ \t]+)?([A-Za-z_]\w*)")
    _INCLUDE = re.compile(r'(?mi)^[ \t]*!include[ \t]+(?:/NONFATAL[ \t]+)?"?([^"\n]+?)"?[ \t]*$')
    ...
    def _extract_entities(self, file_id, text, path):
        ...
        self._record_module_meta(
            file_id, kind="header" if path.suffix.lower() == ".nsh" else "installer",
            sections=len(seen_cls), functions=len(seen_fn), defines=len(seen_var))
