...
class VbScriptAnalyzer(ShellScriptBase):
    ...
    _PROC = re.compile(r"(?mi)^[ \t]*(?:public[ \t]+|private[ \t]+|default[ \t]+)?"
                       r"(sub|function)[ \t]+([A-Za-z_]\w*)[ \t]*(?:\(([^)]*)\))?")
    _CLASS = re.compile(r"(?mi)^[ \t]*class[ \t]+([A-Za-z_]\w*)")
    _DIM = re.compile(r"(?mi)^[ \t]*(dim|const|public|private)[ \t]+([A-Za-z_]\w*)"
                      r"(?:[ \t]*=[ \t]*(.*))?")
    ...
    def _extract_entities(self, file_id, text, path):
        ...
        for m in self._CLASS.finditer(clean):
            ...
            if name not in seen_cls:
                seen_cls[name] = self._add_class(file_id, name,
                                                 description="vbscript class")
        ...
        for m in self._PROC.finditer(clean):
            ...
            params = [p.replace("ByVal", "").replace("ByRef", "").strip()
                      for p in params]
            self._add_shell_function(file_id, name, params=params,
                                     description="vbscript " + kind)
        ...
        self._record_module_meta(file_id, procedures=len(seen_fn),
                                 classes=len(seen_cls), variables=len(seen_var))
...
class WsfAnalyzer(ShellScriptBase):
    ...
    def _extract_entities(self, file_id, text, path):
        ...
        self._record_module_meta(file_id, jobs=len(seen_cls),
