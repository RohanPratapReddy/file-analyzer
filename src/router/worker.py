...
This is the unit of work the Go and Java analysis planes fan out to. Given a
...
It is deliberately a standalone script (``python -m src.router.worker`` or
``python worker.py``) so the Go/Java planes can invoke it as an ordinary
subprocess, one process per shard, achieving genuine OS-level parallelism.
